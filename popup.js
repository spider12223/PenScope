// PenScope v6.0 — Popup Logic
// v6.0 introduces three view modes layered over the same data engine: Classic (v5.9
// verbatim), Red (chain-first attacker view), Blue (defender health dashboard). Modes
// are theme + renderer choices; the underlying state collected by background.js is
// untouched. See the mode router section near the bottom of this file.
let tabId=null,D=null;
// v6.0 — mode state. Persisted to background per-tab via setMode message; restored
// from getData on every load so popup re-opens land on the user's last mode.
const MODES=["classic","red","blue"];
let currentMode="classic";
let lastTabData=null;
let _showRealCookies=false;
function _chooseRepro(c){if(!c)return"";return _showRealCookies&&c._liveReproCmd?c._liveReproCmd:(c.reproCmd||"");}
function _debounce(fn,ms){let t;return function(){const a=arguments,c=this;clearTimeout(t);t=setTimeout(()=>fn.apply(c,a),ms);};}
...
document.addEventListener("DOMContentLoaded",async()=>{const[tab]=await chrome.tabs.query({active:true,currentWindow:true});if(!tab)return;tabId=tab.id;document.getElementById("tgtUrl").textContent=tab.url||"—";
// v6.0 — wire the 3-segment mode toggle. Click handler routes to setMode(); each button
// also responds to keyboard activation (Enter/Space) via the native button semantics.
// We bind on the parent so the listeners survive any future re-renders of the header.
...
chrome.runtime.sendMessage({action:"runScan",tabId},()=>setTimeout(load,600));chrome.runtime.sendMessage({action:"getCookies",tabId});document.querySelectorAll(".tab").forEach(t=>t.addEventListener("click",()=>{document.querySelectorAll(".tab").forEach(x=>x.classList.remove("active"));document.querySelectorAll(".tc").forEach(x=>x.classList.remove("active"));t.classList.add("active");document.getElementById(`t-${t.dataset.t}`).classList.add("active");}));document.getElementById("btnScan").addEventListener("click",()=>{chrome.runtime.sendMessage({action:"runScan",tabId},()=>setTimeout(load,800));});document.getElementById("btnClear").addEventListener("click",()=>{chrome.runtime.sendMessage({action:"clearData",tabId},()=>load());});document.getElementById("btnClaude").addEventListener("click",sendToClaude);
document.getElementById("btnSnap")?.addEventListener("click",()=>{
  chrome.runtime.sendMessage({action:"saveSnapshot",tabId},r=>{
    if(chrome.runtime.lastError){toast("Snap failed: "+chrome.runtime.lastError.message);return;}
    if(r&&r.ok)toast(`📸 Snapshot saved · ${r.count}/20 · score ${r.score}`);
    else toast("Snap failed: "+(r?.error||"unknown"));
  });
});
document.getElementById("btnDiff")?.addEventListener("click",()=>{
  chrome.runtime.sendMessage({action:"diffSnapshots",tabId},r=>{
    if(chrome.runtime.lastError){toast("Diff failed: "+chrome.runtime.lastError.message);return;}
    if(!r||!r.ok){toast("Diff failed: "+(r?.error||"unknown"));return;}
    if(!r.last){toast("No prior snapshot — click 📸 Snap first");return;}
    const d=r.diff||{new:[],resolved:[],unchanged:[]};
    const dt=r.last?new Date(r.last.ts).toLocaleString():"never";
    let body=`Diff vs snapshot from ${dt}\n\n`;
    body+=`+ ${d.new.length} new findings\n`;
    body+=`- ${d.resolved.length} resolved\n`;
    body+=`= ${d.unchanged.length} unchanged\n\n`;
    if(d.new.length){body+="--- NEW ---\n";d.new.slice(0,30).forEach(f=>{body+=`[${(f.severity||"?").toUpperCase()}] ${f.type} @ ${f.where||"?"}\n`;});if(d.new.length>30)body+=`...+${d.new.length-30} more\n`;body+="\n";}
    if(d.resolved.length){body+="--- RESOLVED ---\n";d.resolved.slice(0,15).forEach(f=>{body+=`[${(f.severity||"?").toUpperCase()}] ${f.type} @ ${f.where||"?"}\n`;});if(d.resolved.length>15)body+=`...+${d.resolved.length-15} more\n`;}
    window.alert(body);
  });
});document.getElementById("btnDeep").addEventListener("click",toggleDeep);const expBtn=document.getElementById("btnExport"),expMenu=document.getElementById("exportMenu");expBtn.addEventListener("click",e=>{e.stopPropagation();expMenu.classList.toggle("show");});document.addEventListener("click",()=>expMenu.classList.remove("show"));expMenu.querySelectorAll(".exp-item").forEach(i=>i.addEventListener("click",e=>{e.stopPropagation();exportData(i.dataset.fmt);expMenu.classList.remove("show");}));document.getElementById("fE").addEventListener("input",_debounce(e=>renderEndpoints(D?.endpoints||[],e.target.value),50));document.getElementById("fL").addEventListener("input",_debounce(e=>renderLinks(D?.links||[],e.target.value),50));document.getElementById("fC").addEventListener("input",_debounce(e=>renderConsole(e.target.value),50));document.getElementById("btnProbe").addEventListener("click",toggleProbe);
// Deep dropdown
...
function load(){chrome.runtime.sendMessage({action:"getData",tabId},data=>{if(!data)return;D=data;lastTabData=data;
  // v6.0 — restore mode from the data engine. Background persists `mode` per-tab.
  // First load picks up whatever the user last selected; subsequent toggles update both
  // local state and the persisted state via setMode.
  if(data.mode&&MODES.includes(data.mode)&&data.mode!==currentMode){
    currentMode=data.mode;
    applyModeBodyClass(currentMode);
    syncModeToggleUI(currentMode);
  }
  rerender();
});}

// v6.0 — Render dispatch. The single entry point that routes to the active mode's
// renderer. Called from load() after data arrives, and from setMode() when the user
// switches modes. Each renderer is responsible for showing/hiding the right host
// (classicHost or modeHost) and populating it.
function rerender(){
  if(!lastTabData)return;
  if(currentMode==="red")return renderRed(lastTabData);
  if(currentMode==="blue")return renderBlue(lastTabData);
  return renderClassic(lastTabData);
}

// v6.0 — Classic renderer. Pure rename + behavior preservation: this is the v5.9
// rendering pathway with zero changes. Mounted by toggling classicHost back on and
// emptying modeHost. Every existing v5.9 capability flows through here.
function renderClassic(data){
  document.getElementById("classicHost").style.display="";
  const mh=document.getElementById("modeHost");
  if(mh){mh.style.display="none";mh.innerHTML="";}
  updateDeepUI(data.deepEnabled);
  updateProbeUI(data.probeData);
  updateStats();
  renderEndpoints(data.endpoints||[]);
  renderSecrets(data.secrets||[]);
  renderHidden(data.hiddenFields||[]);
  renderHeaders(data.headers||[]);
  renderForms(data.forms||[]);
  renderTech(data.techStack||[]);
  renderStorage(data.storageData||{});
...
// v6.0 — Red renderer. Chain-first attacker view: exploit chain rail at top with
// per-chain weaponize accordion (curl / Nuclei / Burp / H1 draft / Claude queue),
// classic tabs collapsed below, and a Claude bidirectional sync banner that appears
// when the user has piped a queue back from Claude.
//
// Key invariant: this populates the classic DOM SILENTLY first (so cloning the rendered
// content into accordion sections later just works), then layers the red UI on top.
// The classic DOM is hidden via `display:none` on classicHost; nothing about the data
// pipeline changes between modes.
function renderRed(data){
  // Step 1 — populate the classic DOM silently. Chain-first view layers atop, but
  // expanding a "classic section" accordion below grabs nodes from these populated IDs.
  // Without this step, the accordion would expand to an empty container the first time.
  updateDeepUI(data.deepEnabled);
  updateProbeUI(data.probeData);
  updateStats();
  renderEndpoints(data.endpoints||[]);
  ...
  // Step 2 — hide classic, show modeHost.
  document.getElementById("classicHost").style.display="none";
  const mh=document.getElementById("modeHost");
  if(!mh)return;
  mh.style.display="";

  const chains=Array.isArray(data.exploitChains)?data.exploitChains.slice():[];
  // Sort by severity ordinal × confidence (descending). High-confidence criticals first.
  const sevOrd={critical:4,high:3,medium:2,low:1,info:0};
  chains.sort((a,b)=>((sevOrd[b.severity]||0)*(b.confidence||0.5))-((sevOrd[a.severity]||0)*(a.confidence||0.5)));

  const queue=Array.isArray(data.claudeQueue)?data.claudeQueue:[];
  const queueLen=queue.length;

  // Compose modeHost
  let h="";

  // ---- Claude queue banner (only when queue is non-empty) ----
  // The queue is what `Sync from Claude` parses out of the clipboard. Run queue fires
  // a probe pass against each queued attack. Dismiss removes the queue without running.
  if(queueLen){
    h+=`<div id="rdClaudeBanner" style="padding:10px 16px;background:linear-gradient(90deg,rgba(217,119,6,.18),rgba(255,107,90,.06));border-bottom:1px solid rgba(217,119,6,.25);display:flex;align-items:center;gap:10px">
      <span style="font-size:11px;font-weight:700;color:var(--coral)">Claude queue: ${queueLen} attack${queueLen===1?"":"s"} pending</span>
      <span style="flex:1;font-size:10px;color:var(--t2)">Pasted from Claude. Click run to execute against the live target with current probe context.</span>
      <button class="btn v6-focusable" id="rdClaudeRun" style="background:linear-gradient(135deg,#d97706,var(--coral));color:#fff;border:none;font-weight:700;padding:6px 12px" aria-label="Run Claude attack queue against live target">Run queue ▶</button>
      <button class="btn v6-focusable" id="rdClaudeClear" aria-label="Dismiss Claude queue without running">Dismiss</button>
    </div>`;
  }

  // ---- Red action bar — Claude sync controls visible only in Red mode ----
  // The existing → Claude button (in the classic header) still works to push findings
  // OUT to Claude. This bar adds the inverse: pulling Claude's suggested attacks IN.
  h+=`<div style="padding:10px 16px;display:flex;gap:8px;align-items:center;border-bottom:1px solid var(--glassbrd);flex-wrap:wrap">
    <span style="font-size:10px;color:var(--t3);text-transform:uppercase;letter-spacing:.5px;font-weight:700">Red ops</span>
    <button class="btn btn-claude v6-focusable" id="rdSendClaude" aria-label="Push findings to Claude">→ Claude</button>
    <button class="btn v6-focusable" id="rdSyncClaude" aria-label="Sync attack queue from Claude clipboard" style="border-color:rgba(217,119,6,.4);color:var(--coral)">⟳ Sync from Claude</button>
    <label style="display:inline-flex;align-items:center;gap:5px;font-size:10px;color:${_showRealCookies?'var(--red)':'var(--t3)'};cursor:pointer;padding:4px 8px;border:1px solid ${_showRealCookies?'rgba(255,58,92,.4)':'var(--glassbrd2)'};border-radius:6px;background:${_showRealCookies?'rgba(255,58,92,.06)':'transparent'};font-weight:${_showRealCookies?'700':'500'}" title="When ON, repro commands display real session cookies. Default OFF: cookies shown as <REDACTED-name>. Use only when generating commands you'll run yourself; never share screenshots with this on.">
      <input type="checkbox" id="rdShowReal" ${_showRealCookies?'checked':''} aria-label="Show real cookie values in repro commands" style="margin:0;cursor:pointer">
      ${_showRealCookies?'⚠ Real cookies':'Show real values'}
    </label>
    <span style="flex:1"></span>
    <span style="font-size:10px;color:var(--t3);font-family:var(--mono)">${chains.length} chains · ${(data.endpoints||[]).length} endpoints · ${(data.secrets||[]).length} secrets</span>
  </div>`;

  // ---- Chain rail ----
  h+=`<div style="padding:8px 16px 4px;font-size:10px;color:var(--t3);text-transform:uppercase;letter-spacing:.6px;font-weight:700">Exploit chains</div>`;
  if(!chains.length){
    h+=`<div style="padding:24px 16px;text-align:center;color:var(--t3);font-size:11px">No chains yet. Run probe (Deep + Probe button) to surface compound exploit paths.</div>`;
  }else{
    h+=`<div id="rdChainRail" role="list" aria-label="Exploit chains">`;
    chains.forEach((chain,idx)=>{
      h+=renderRedChainRow(chain,idx);
    });
    h+=`</div>`;
  }

  // ---- Classic tabs as collapsed accordion ----
  // Each accordion section, when expanded, mounts a copy of the corresponding classic
  // tab's container so users can drill into endpoints/secrets/etc. without leaving Red mode.
  h+=`<div style="padding:14px 16px 6px;font-size:10px;color:var(--t3);text-transform:uppercase;letter-spacing:.6px;font-weight:700;border-top:1px solid var(--glassbrd);margin-top:8px">Reference data</div>`;
  const refSections=[
    {key:"endpoints",label:"Endpoints",count:(data.endpoints||[]).length,sourceId:"t-endpoints"},
    {key:"secrets",label:"Secrets",count:(data.secrets||[]).length,sourceId:"t-secrets"},
    {key:"hidden",label:"Hidden fields",count:(data.hiddenFields||[]).length,sourceId:"t-hidden"},
    {key:"headers",label:"Headers / CSP / cookies",count:(data.headers||[]).length,sourceId:"t-headers"},
    {key:"forms",label:"Forms",count:(data.forms||[]).length,sourceId:"t-forms"},
    {key:"tech",label:"Tech stack",count:(data.techStack||[]).length,sourceId:"t-tech"},
    {key:"storage",label:"Storage",count:Object.keys(data.storageData?.local||{}).length+Object.keys(data.storageData?.session||{}).length,sourceId:"t-storage"},
    {key:"links",label:"Links",count:(data.links||[]).length,sourceId:"t-links"},
    {key:"deep",label:"Deep findings",count:(data.responseBodies||[]).length,sourceId:"t-deep"},
    {key:"console",label:"Console",count:(data.consoleLogs||[]).length,sourceId:"t-console"},
  ];
  h+=`<div id="rdRefSections">`;
  refSections.forEach(s=>{
    h+=`<div class="rd-ref-row" data-rdref="${escA(s.sourceId)}" style="border-top:1px solid var(--glassbrd)">
      <button class="v6-focusable" data-rdref-toggle="${escA(s.sourceId)}" aria-expanded="false" aria-controls="rdref-body-${escA(s.key)}" style="width:100%;background:transparent;border:none;color:var(--t1);font-family:var(--font);font-size:11px;font-weight:600;padding:9px 0;display:flex;align-items:center;gap:8px;cursor:pointer;text-align:left">
        <span style="color:var(--t3);font-family:var(--mono);font-size:10px;width:14px">▸</span>
        <span style="flex:1">${esc(s.label)}</span>
        <span style="font-family:var(--mono);font-size:9px;color:var(--t3);background:var(--glass2);padding:2px 7px;border-radius:8px">${s.count}</span>
      </button>
      <div class="rd-ref-body" id="rdref-body-${escA(s.key)}" style="display:none;padding:0 0 8px"></div>
    </div>`;
  });
  h+=`</div>`;
  ...
  // ---- Wiring ----
  // Chain expand: only one chain expanded at a time (accordion behavior).
  mh.querySelectorAll(".rd-chain-head").forEach(btn=>{
    btn.addEventListener("click",()=>{
      const chainId=btn.getAttribute("data-chainid");
      const wasOpen=btn.getAttribute("aria-expanded")==="true";
      // Collapse all
      mh.querySelectorAll(".rd-chain-head").forEach(b=>{
        b.setAttribute("aria-expanded","false");
        const arrow=b.querySelector(".rd-chain-chev");if(arrow)arrow.textContent="▸";
      });
      mh.querySelectorAll(".rd-chain-body").forEach(b=>{b.style.display="none";});
      // Toggle this one
      if(!wasOpen){
        btn.setAttribute("aria-expanded","true");
        const arrow=btn.querySelector(".rd-chain-chev");if(arrow)arrow.textContent="▾";
        const body=mh.querySelector(`#rdchain-body-${CSS.escape(chainId)}`);
        if(body)body.style.display="";
      }
    });
  });

  // Per-chain weaponize buttons. Each emits a copy-to-clipboard action with a toast.
  mh.querySelectorAll("[data-rdwpn]").forEach(btn=>{
    btn.addEventListener("click",e=>{
      e.stopPropagation();
      const action=btn.getAttribute("data-rdwpn");
      const chainId=btn.getAttribute("data-chainid");
      const chain=chains.find(c=>String(c.id||chains.indexOf(c))===chainId);
      if(!chain)return;
      runRedWeaponize(action,chain,data);
    });
  });

  // Reference accordion expand/collapse — moves the actual classic content node into
  // the accordion body so click handlers + filter inputs continue to work. When the
  // section is collapsed again, the node is moved back to its hidden classic home.
  mh.querySelectorAll("[data-rdref-toggle]").forEach(btn=>{
    btn.addEventListener("click",()=>{
      const sourceId=btn.getAttribute("data-rdref-toggle");
      const wasOpen=btn.getAttribute("aria-expanded")==="true";
      const arrow=btn.querySelector("span");
      const body=btn.nextElementSibling;
      if(!body)return;
      if(wasOpen){
        // Collapse — return any borrowed node to classicHost
        btn.setAttribute("aria-expanded","false");
        if(arrow)arrow.textContent="▸";
        body.style.display="none";
        const borrowed=body.firstElementChild;
        if(borrowed){
          const home=document.getElementById(sourceId);
          if(home)home.appendChild(borrowed);
        }
      }else{
        // Expand — move source node from hidden classic home into the accordion body
        btn.setAttribute("aria-expanded","true");
        if(arrow)arrow.textContent="▾";
        body.style.display="";
        const home=document.getElementById(sourceId);
        if(home&&home.firstElementChild){
          // We move the inner content (input + result container), preserving listeners
          while(home.firstChild)body.appendChild(home.firstChild);
        }
      }
    });
  });

  // Claude push (existing flow unchanged)
  const sendBtn=document.getElementById("rdSendClaude");
  if(sendBtn)sendBtn.addEventListener("click",sendToClaude);

  // Claude sync — read clipboard, parse fenced JSON, persist queue, re-render
  const syncBtn=document.getElementById("rdSyncClaude");
  if(syncBtn)syncBtn.addEventListener("click",syncFromClaude);

  // Run/dismiss queue
  const runQ=document.getElementById("rdClaudeRun");
  if(runQ)runQ.addEventListener("click",()=>runClaudeQueue());
  const clearQ=document.getElementById("rdClaudeClear");
  if(clearQ)clearQ.addEventListener("click",()=>clearClaudeQueue());

  const showReal=document.getElementById("rdShowReal");
  if(showReal)showReal.addEventListener("change",e=>{
    if(e.target.checked){
      const ok=window.confirm("Reveal real session cookies in repro commands?\n\nWith this ON, copying a curl/Burp/Nuclei artifact embeds your live session cookies. Never share screenshots, screen-recordings, or clipboard content with anyone while this is enabled. The default OFF state shows <REDACTED-name> placeholders.\n\nProceed?");
      if(!ok){e.target.checked=false;return;}
    }
    _showRealCookies=e.target.checked;
    rerender();
  });
...
// Render a single chain row + its hidden weaponize body. Body becomes visible on expand.
function renderRedChainRow(chain,idx){
  ...
  const id=String(chain.id!=null?chain.id:idx);
  let row=`<div class="rd-chain-row" role="listitem" style="border-top:1px solid var(--glassbrd);border-left:3px solid ${sevColor}">
    <button class="rd-chain-head v6-focusable" data-chainid="${escA(id)}" aria-expanded="false" aria-controls="rdchain-body-${escA(id)}" style="width:100%;background:transparent;border:none;color:var(--t1);font-family:var(--font);padding:10px 14px;display:flex;align-items:center;gap:10px;cursor:pointer;text-align:left">
      <span style="font-size:10px;font-weight:800;color:${sevColor};padding:2px 7px;border-radius:4px;background:${sevBg};text-transform:uppercase;letter-spacing:.4px;min-width:64px;text-align:center">${esc(chain.severity||"info")}</span>
      <span style="flex:1;font-size:12px;font-weight:700">${esc(chain.title||"(untitled chain)")}</span>
      <span style="font-family:var(--mono);font-size:10px;color:var(--t3)">${conf}%</span>
      <span class="rd-chain-chev" style="font-family:var(--mono);font-size:11px;color:var(--t3);width:14px;text-align:right">▸</span>
    </button>
    <div class="rd-chain-body" id="rdchain-body-${escA(id)}" style="display:none;padding:8px 16px 14px;border-top:1px solid var(--glassbrd);background:var(--bgh)">`;

  // Summary
  if(chain.summary){
    row+=`<div style="font-size:11px;color:var(--t2);line-height:1.6;margin-bottom:8px">${esc(chain.summary)}</div>`;
  }

  if(chain.reproCmd){
    const _repro=_chooseRepro(chain);
    const _hasLive=!!chain._liveReproCmd;
    const _badge=_hasLive?(_showRealCookies?'<span style="font-size:9px;color:var(--red);font-weight:700;margin-left:6px">LIVE COOKIES</span>':'<span style="font-size:9px;color:var(--t3);font-weight:600;margin-left:6px">REDACTED</span>'):"";
    row+=`<div style="font-family:var(--mono);font-size:10px;color:var(--green);background:rgba(58,255,138,.05);padding:8px 10px;border-radius:8px;border:1px solid rgba(58,255,138,.15);white-space:pre-wrap;word-break:break-all;cursor:pointer;margin-bottom:10px" data-copy="${escA(_repro)}" title="Click to copy">${esc(_repro.substring(0,800))}${_badge}</div>`;
  }

  // Weaponize button row. Each button copies the right artefact to clipboard.
  row+=`<div style="display:flex;flex-wrap:wrap;gap:6px;margin-bottom:10px">
    <button class="btn v6-focusable" data-rdwpn="curl" data-chainid="${escA(id)}" aria-label="Copy curl command">Copy curl</button>
    <button class="btn v6-focusable" data-rdwpn="nuclei" data-chainid="${escA(id)}" aria-label="Copy Nuclei YAML template for this chain">Nuclei YAML</button>
    <button class="btn v6-focusable" data-rdwpn="burp" data-chainid="${escA(id)}" aria-label="Copy Burp raw HTTP request">Burp request</button>
    <button class="btn v6-focusable" data-rdwpn="h1" data-chainid="${escA(id)}" aria-label="Draft a HackerOne report for this chain">Draft H1 report</button>
    <button class="btn v6-focusable" data-rdwpn="claude" data-chainid="${escA(id)}" aria-label="Append chain to Claude queue payload">Send to Claude queue</button>
  </div>`;

  // Next steps
  if(chain.nextSteps&&chain.nextSteps.length){
    row+=`<div style="font-size:10px;color:var(--t3);text-transform:uppercase;letter-spacing:.5px;font-weight:700;margin-bottom:4px">Next steps</div>`;
    row+=`<ul style="list-style:none;padding:0;margin:0">`;
    chain.nextSteps.slice(0,8).forEach(step=>{
      row+=`<li style="padding:2px 0 2px 14px;font-size:11px;color:var(--t2);position:relative;line-height:1.5"><span style="position:absolute;left:0;color:var(--purple)">→</span>${esc(step)}</li>`;
    });
    row+=`</ul>`;
  }
...
// Weaponize action dispatcher. Each branch builds the appropriate artefact and copies
// it to the clipboard with a toast. Generic fallbacks where chain data is sparse.
function runRedWeaponize(action,chain,data){
  let payload="";let label="";
  if(action==="curl"){
    payload=_chooseRepro(chain)||`curl -i "${currentBaseUrl()}${(chain.findings&&chain.findings[0]&&chain.findings[0].path)||"/"}"`;
    label=_showRealCookies&&chain._liveReproCmd?"curl (LIVE COOKIES)":"curl (redacted)";
  }else if(action==="nuclei"){
    payload=buildNucleiForChain(chain);
    label="Nuclei YAML";
  }else if(action==="burp"){
    payload=buildBurpFromChain(chain);
    label="Burp request";
  }else if(action==="h1"){
    payload=buildH1ReportFromChain(chain,data);
    label="H1 report";
  }else if(action==="claude"){
    payload=buildClaudePayloadForChain(chain);
    label="Claude payload";
  }
  if(!payload){toast("Nothing to copy");return;}
  navigator.clipboard.writeText(payload).then(()=>toast("Copied "+label),()=>toast("Clipboard blocked"));
}

// Find a usable base URL for synthesizing requests. Prefers tab URL → first endpoint host.
function currentBaseUrl(){
  try{
    if(D&&D.url){const u=new URL(D.url);return u.origin;}
  }catch(e){/* fall through */}
  if(D&&D.endpoints&&D.endpoints[0]){
    try{return new URL(D.endpoints[0].url).origin;}catch(e){/* fall through */}
  }
  return "https://target";
}

// Build a minimal Nuclei template scoped to this single chain. Reuses the existing
// chain.reproCmd to derive method+path; the structure mirrors what /export-nuclei does.
function buildNucleiForChain(chain){
  const path=(chain.findings&&chain.findings[0]&&chain.findings[0].path)||"/";
  const method=(chain.findings&&chain.findings[0]&&chain.findings[0].method)||"GET";
  const sevLine=chain.severity||"info";
  const id=("penscope-chain-"+(chain.id||"")+"-"+(chain.title||"")).toLowerCase().replace(/[^a-z0-9-]+/g,"-").substring(0,80).replace(/^-+|-+$/g,"");
  const title=(chain.title||"PenScope chain").replace(/"/g,'\\"');
  const summary=(chain.summary||"").replace(/"/g,'\\"').substring(0,500);
  return `id: ${id||"penscope-chain"}\n\ninfo:\n  name: ${title}\n  author: penscope\n  severity: ${sevLine}\n  description: |\n    ${summary}\n  tags: penscope,chain\n\nrequests:\n  - method: ${method}\n    path:\n      - "{{BaseURL}}${path}"\n    matchers-condition: and\n    matchers:\n      - type: status\n        status:\n          - 200\n      - type: word\n        words:\n          - "<<RESPONSE_INDICATOR>>"   # adjust based on chain evidence\n        part: body\n`;
}

// Build a Burp-pasteable raw HTTP request from chain.reproCmd or chain.findings.
// Real-world chain.reproCmd often starts with `curl -i "…"` — we parse method/url/body
// from it where possible and fall back to a synthesized GET.
function buildBurpFromChain(chain){
  const base=currentBaseUrl();
  const finding=chain.findings&&chain.findings[0];
  let method=(finding&&finding.method)||"GET";
  let path=(finding&&finding.path)||"/";
  let body="";
  if(chain.reproCmd){
    const mm=chain.reproCmd.match(/curl\s+(?:-i\s+)?(?:-X\s+(\w+)\s+)?["']([^"']+)["']/);
    if(mm){if(mm[1])method=mm[1].toUpperCase();
      try{const u=new URL(mm[2]);path=u.pathname+u.search;}catch(e){if(mm[2].startsWith("/"))path=mm[2];}
    }
    const bm=chain.reproCmd.match(/-d\s+["']([^"']+)["']/);
    if(bm)body=bm[1];
  }
  let host="target";try{host=new URL(base).host;}catch(e){}
  let raw=`${method} ${path} HTTP/1.1\r\nHost: ${host}\r\nUser-Agent: Mozilla/5.0\r\nAccept: */*\r\nAccept-Language: en-US,en;q=0.9\r\nConnection: close\r\n`;
  if(body){raw+=`Content-Type: application/json\r\nContent-Length: ${body.length}\r\n`;}
  raw+=`Authorization: Bearer <PASTE-TOKEN>\r\nCookie: <PASTE-SESSION>\r\n\r\n`;
  if(body)raw+=body;
  return raw;
}

// Draft a HackerOne-style report in markdown for this chain. Pulls a remediation hint
// from the (Phase 4) blue-fixes module if available; otherwise emits a generic fix
// guidance block keyed on chain severity.
function buildH1ReportFromChain(chain,data){
  const tgt=(data&&data.url)||currentBaseUrl();
  const sev=(chain.severity||"medium").toUpperCase();
  const cvss={critical:"9.0+",high:"7.0–8.9",medium:"4.0–6.9",low:"0.1–3.9",info:"N/A"}[chain.severity||"medium"]||"4.0–6.9";
  const findingType=chain.findingType||chain.id||"";
  let fixHint="";
  // Phase 4: if FIX_SNIPPETS is loaded, look up a remediation snippet.
  if(typeof getFixForFinding==="function"){
    try{
      const fx=getFixForFinding({type:findingType,id:chain.id});
      if(fx)fixHint=`### Suggested fix\n\n${fx.title}\n\n${fx.why||""}\n\n\`\`\`\n${fx.snippet_raw||""}\n\`\`\``;
    }catch(e){/* phase 4 not loaded yet */}
  }
  if(!fixHint){
    fixHint=`### Suggested fix\n\nApply standard remediation for ${chain.title||"this finding"}: enforce server-side authorization on the affected endpoint, validate input, and add coverage for this case in your security regression suite.`;
  }
  return `# ${chain.title||"PenScope finding"}\n\n**Severity:** ${sev} (CVSS estimate: ${cvss})\n**Target:** ${tgt}\n\n## Summary\n\n${chain.summary||"PenScope identified a compound exploit chain combining multiple findings."}\n\n## Steps to reproduce\n\n\`\`\`\n${chain.reproCmd||"(see PenScope export)"}\n\`\`\`\n\n## Impact\n\nThis chain combines ${(chain.findings||[]).length} signals into a single exploit path. ${chain.severity==="critical"?"A successful attacker would gain unauthorized access to sensitive functionality or data with high confidence.":chain.severity==="high"?"Successful exploitation would bypass intended access controls and expose sensitive data or operations.":"Successful exploitation would weaken the security posture and provide a foothold for further attacks."}\n\n${fixHint}\n\n## Detection\n\nIdentified by PenScope v6.0 attack chain correlator with confidence ${Math.round((chain.confidence||0.5)*100)}%.\n`;
}

// Build a Claude-bound JSON payload for a single chain. Inverse-shape of the
// penscope-queue format: this is what the user pastes back to Claude when asking for
// follow-up attack ideas about a specific chain.
function buildClaudePayloadForChain(chain){
  const obj={
    version:1,
    context:"penscope-chain-deep-dive",
    chain:{
      id:chain.id||null,
      severity:chain.severity||"info",
      title:chain.title||"",
      summary:chain.summary||"",
      reproCmd:chain.reproCmd||"",
      confidence:chain.confidence||0.5,
      findings:(chain.findings||[]).slice(0,8),
      nextSteps:(chain.nextSteps||[]).slice(0,8),
    },
    ask:"Suggest 3-6 follow-up attacks specific to this chain. Reply with the standard penscope-queue fenced JSON format."
  };
  return "```penscope-chain\n"+JSON.stringify(obj,null,2)+"\n```";
}

// ---- Claude bidirectional sync ----
// User pipes findings out via → Claude (existing flow). User asks Claude for follow-up
// attacks. Claude responds with a fenced ```penscope-queue block. User clicks Sync from
// Claude — we read clipboard, regex-find the block, validate, persist to background.
async function syncFromClaude(){
  let text="";
  try{text=await navigator.clipboard.readText();}catch(e){toast("Clipboard read denied — copy queue and try again");return;}
  const queue=parseClaudeQueueFromClipboard(text);
  if(!queue){toast("Couldn't parse Claude queue — check the format.");return;}
  if(!queue.attacks||!queue.attacks.length){toast("Queue parsed but contains no attacks");return;}
  // Persist to background — getData on next render will reflect it
  try{
    chrome.runtime.sendMessage({action:"setClaudeQueue",tabId,queue:queue.attacks},()=>{
      toast(`Queue synced — ${queue.attacks.length} attack${queue.attacks.length===1?"":"s"}`);
      setTimeout(load,250);
    });
  }catch(e){toast("Sync failed — "+(e.message||"unknown"));}
}

// Strict parser: looks for a ```penscope-queue fenced block, parses JSON, validates the
// {version, attacks[]} shape. Tolerant of surrounding prose. Returns null on any error
// so the caller can show "Couldn't parse" without leaking parse details.
function parseClaudeQueueFromClipboard(text){
  if(!text||typeof text!=="string")return null;
  // Match ```penscope-queue ... ``` (case-insensitive label, multi-line JSON inside)
  const fence=text.match(/```penscope-queue\s*\n([\s\S]*?)```/i);
  let json=fence?fence[1]:null;
  if(!json){
    // Tolerate Claude using ```json after a "penscope-queue" hint nearby
    const looseFence=text.match(/```(?:json)?\s*\n(\{[\s\S]*?"attacks"[\s\S]*?\})\s*```/i);
    if(looseFence)json=looseFence[1];
  }
  if(!json)return null;
  try{
    const obj=JSON.parse(json.trim());
    if(!obj||typeof obj!=="object")return null;
    if(!Array.isArray(obj.attacks))return null;
    // Validate each attack — tolerant: type is required, the rest is shape-flexible
    const cleaned=obj.attacks.filter(a=>a&&typeof a==="object"&&typeof a.type==="string"&&a.type.length<60).map(a=>({
      type:a.type,
      endpoint:typeof a.endpoint==="string"?a.endpoint.substring(0,500):undefined,
      url:typeof a.url==="string"?a.url.substring(0,500):undefined,
      method:typeof a.method==="string"?a.method.substring(0,12).toUpperCase():undefined,
      body:typeof a.body==="string"?a.body.substring(0,8000):undefined,
      headers:(a.headers&&typeof a.headers==="object")?a.headers:undefined,
      field:typeof a.field==="string"?a.field.substring(0,200):undefined,
      rationale:typeof a.rationale==="string"?a.rationale.substring(0,800):undefined,
      severity_hint:typeof a.severity_hint==="string"?a.severity_hint.substring(0,20):undefined,
    })).slice(0,40);// hard cap so a runaway queue can't DoS the popup
    if(!cleaned.length)return null;
    return {version:obj.version||1,attacks:cleaned};
  }catch(e){return null;}
}

// Run the queued attacks via the probe engine. Background's startProbe accepts a queue
// override (Phase 2 shim) and runs each attack with current custom headers/stealth.
function runClaudeQueue(){
  if(!D||!D.claudeQueue||!D.claudeQueue.length){toast("Queue is empty");return;}
  if(!document.getElementById("btnDeep")||!document.getElementById("btnDeep").classList.contains("active")){toast("Enable Deep mode first");return;}
  const hdrEl=document.getElementById("probeHeaders");
  ...
  toast("Running Claude queue...");
  chrome.runtime.sendMessage({action:"runClaudeQueue",tabId,customHeaders,stealth},r=>{
    if(r&&r.ok){
      toast(`Queue done — ${r.results?.length||0} attacks executed`);
      setTimeout(load,400);
    }else{
      toast("Queue error — check Deep tab");
      setTimeout(load,300);
    }
  });
}
...
function clearClaudeQueue(){
  chrome.runtime.sendMessage({action:"setClaudeQueue",tabId,queue:[]},()=>{
    toast("Queue cleared");
    setTimeout(load,200);
  });
}

// v6.0 — Blue renderer. Defender health dashboard:
//   - Health score (0-100) computed from severity-weighted findings, updated live as
//     findings get marked fixed.
//   - 6 stat tiles (critical/high/medium/low/info/total) for quick triage.
//   - Top-5 fixes prioritized by severity × ease, each with a working snippets panel.
//   - Action row: Generate CSP, Compliance Audit, Snapshot, Compare, Continuous monitor.
//   - Reference accordion (Drift, all findings, classic data) below the action row.
function renderBlue(data){
  // Populate classic DOM silently so the reference accordion can borrow rendered nodes
  updateDeepUI(data.deepEnabled);
  updateProbeUI(data.probeData);
  updateStats();
  renderEndpoints(data.endpoints||[]);
  ...
  // Aggregate findings — every PenScope source has a different shape, so normalize first
  const allFindings=collectBlueFindings(data);
  const markedFixed=new Set((data.markedFixed||[]).map(f=>f.id||f));
  const live=allFindings.filter(f=>!markedFixed.has(f.id));
  const counts=countFindingsBySeverity(live);
  const score=computeHealthScore(counts);
  const prevSnapshot=loadLastSnapshotMeta(data.url);
  const trend=prevSnapshot?(score-(prevSnapshot.score||score)):0;
  const trendArrow=trend>0?"↑":trend<0?"↓":"·";
  const trendColor=trend>0?"var(--green)":trend<0?"var(--red)":"var(--t3)";

  // Top 5 fixes — severity × ease, with a working FIX_SNIPPETS lookup
  const topFixes=prioritizeFixes(live,5);

  let h="";
  // Header row: score + trend
  h+=`<div style="padding:14px 16px;border-bottom:1px solid var(--glassbrd);display:flex;align-items:center;gap:14px">
    <div>
      <div style="font-size:9px;color:var(--t3);text-transform:uppercase;letter-spacing:.6px;font-weight:700">Health score</div>
      <div style="font-size:26px;font-weight:800;font-family:var(--mono);color:${score>=80?"var(--green)":score>=60?"var(--yellow)":score>=40?"var(--orange)":"var(--red)"};line-height:1.1">${score}<span style="font-size:14px;color:var(--t3);font-weight:500">/100</span></div>
    </div>
    <div style="font-size:11px;color:${trendColor};font-family:var(--mono)">${trendArrow} ${trend>0?"+":""}${trend} vs last</div>
    <div style="flex:1"></div>
    <div style="display:grid;grid-template-columns:repeat(6,minmax(50px,1fr));gap:5px">
      ${blueStatTile("CRIT",counts.critical,"var(--red)")}
      ${blueStatTile("HIGH",counts.high,"var(--orange)")}
      ${blueStatTile("MED",counts.medium,"var(--yellow)")}
      ${blueStatTile("LOW",counts.low,"var(--blue)")}
      ${blueStatTile("INFO",counts.info,"var(--t3)")}
      ${blueStatTile("TOT",counts.total,"var(--purple)")}
    </div>
  </div>`;

  // Top 5 fixes
  h+=`<div style="padding:12px 16px 6px;font-size:10px;color:var(--t3);text-transform:uppercase;letter-spacing:.6px;font-weight:700">Top 5 fixes this sprint</div>`;
  if(!topFixes.length){
    h+=`<div style="padding:14px 16px;color:var(--t3);font-size:11px">No actionable findings — score is ${score}/100. Either you have no issues or PenScope hasn't gathered enough data yet.</div>`;
  }else{
    h+=`<div id="bluTopFixes">`;
    topFixes.forEach((f,i)=>{
      h+=renderBlueFixRow(f,i);
    });
    h+=`</div>`;
  }

  // Action bar
  h+=`<div style="padding:10px 16px;display:flex;flex-wrap:wrap;gap:6px;border-top:1px solid var(--glassbrd);border-bottom:1px solid var(--glassbrd);background:var(--bgh)">
    <button class="btn v6-focusable" id="bluGenCsp" aria-label="Generate tight CSP from observed traffic">Generate CSP</button>
    <button class="btn v6-focusable" id="bluCompliance" aria-label="Compliance audit across PCI, NESA, SAMA, ISO">Compliance Audit</button>
    <button class="btn v6-focusable" id="bluSnap" aria-label="Save snapshot of current scan">Snapshot</button>
    <button class="btn v6-focusable" id="bluCompare" aria-label="Compare to last snapshot">Compare to last</button>
    <span style="flex:1"></span>
    <button class="btn v6-focusable" id="bluMonitor" aria-label="Toggle continuous monitor for new secret leaks">${data.continuousMonitor&&data.continuousMonitor.enabled?"⏸ Continuous monitor (ON)":"▶ Continuous monitor"}</button>
  </div>`;

  // Inline panels (CSP, compliance, diff) mount here when the buttons are clicked
  h+=`<div id="bluPanels"></div>`;

  // Reference accordion (drift + classic tabs) — same pattern as Red mode
  h+=`<div style="padding:14px 16px 6px;font-size:10px;color:var(--t3);text-transform:uppercase;letter-spacing:.6px;font-weight:700;border-top:1px solid var(--glassbrd)">Reference</div>`;
  const refSections=[
    {key:"endpoints",label:"Endpoints",count:(data.endpoints||[]).length,sourceId:"t-endpoints"},
    {key:"secrets",label:"Secrets",count:(data.secrets||[]).length,sourceId:"t-secrets"},
    {key:"hidden",label:"Hidden fields",count:(data.hiddenFields||[]).length,sourceId:"t-hidden"},
  ...
  h+=`<div id="bluRefSections">`;
  refSections.forEach(s=>{
    h+=`<div class="blu-ref-row" style="border-top:1px solid var(--glassbrd)">
      <button class="v6-focusable" data-bluref-toggle="${escA(s.sourceId)}" aria-expanded="false" style="width:100%;background:transparent;border:none;color:var(--t1);font-family:var(--font);font-size:11px;font-weight:600;padding:9px 0;display:flex;align-items:center;gap:8px;cursor:pointer;text-align:left">
        <span style="color:var(--t3);font-family:var(--mono);font-size:10px;width:14px">▸</span>
        <span style="flex:1">${esc(s.label)}</span>
        <span style="font-family:var(--mono);font-size:9px;color:var(--t3);background:var(--glass2);padding:2px 7px;border-radius:8px">${s.count}</span>
      </button>
      <div class="blu-ref-body" style="display:none;padding:0 0 8px"></div>
    </div>`;
  });
  h+=`</div>`;
  ...
  // Wiring
  // Top-fix expand
  mh.querySelectorAll("[data-blufix-toggle]").forEach(btn=>{
    btn.addEventListener("click",()=>{
      const wasOpen=btn.getAttribute("aria-expanded")==="true";
      mh.querySelectorAll("[data-blufix-toggle]").forEach(b=>b.setAttribute("aria-expanded","false"));
      mh.querySelectorAll(".blu-fix-body").forEach(b=>b.style.display="none");
      if(!wasOpen){
        btn.setAttribute("aria-expanded","true");
        const body=btn.nextElementSibling;if(body)body.style.display="";
      }
    });
  });
  // Fix language tabs
  mh.querySelectorAll("[data-bluvariant]").forEach(btn=>{
    btn.addEventListener("click",e=>{
      e.stopPropagation();
      const variant=btn.getAttribute("data-bluvariant");
      const fixIdx=btn.getAttribute("data-fixidx");
      const wrap=btn.closest(".blu-fix-body");
      if(!wrap)return;
      wrap.querySelectorAll("[data-bluvariant]").forEach(b=>b.classList.remove("active"));
      btn.classList.add("active");
      const codeEl=wrap.querySelector(".blu-fix-code");
      const fix=topFixes[+fixIdx]&&topFixes[+fixIdx].fix;
      if(codeEl&&fix){
        const code=fix[variant]||fix.snippet_raw||"(no snippet)";
        codeEl.textContent=code;
      }
    });
  });
  // Mark as fixed
  mh.querySelectorAll("[data-blu-markfix]").forEach(btn=>{
    btn.addEventListener("click",e=>{
      e.stopPropagation();
      const fid=btn.getAttribute("data-blu-markfix");
      chrome.runtime.sendMessage({action:"markFixed",tabId,findingId:fid},()=>{
  ...
  // Copy snippet
  mh.querySelectorAll("[data-blu-copysnip]").forEach(btn=>{
    btn.addEventListener("click",e=>{
      e.stopPropagation();
      const wrap=btn.closest(".blu-fix-body");
      const codeEl=wrap?wrap.querySelector(".blu-fix-code"):null;
      if(codeEl){
        navigator.clipboard.writeText(codeEl.textContent||"").then(()=>toast("Copied snippet"),()=>toast("Clipboard blocked"));
      }
    });
  });
  // Action bar
  document.getElementById("bluGenCsp")?.addEventListener("click",()=>showCSPPanel(data));
  document.getElementById("bluCompliance")?.addEventListener("click",()=>showCompliancePanel(data,live));
  document.getElementById("bluSnap")?.addEventListener("click",()=>{
    chrome.runtime.sendMessage({action:"saveSnapshot",tabId},r=>{
      if(r&&r.ok)toast("Snapshot saved at "+new Date().toLocaleTimeString());
      else toast("Snapshot failed");
    });
  });
  document.getElementById("bluCompare")?.addEventListener("click",()=>showDiffPanel(data));
  document.getElementById("bluMonitor")?.addEventListener("click",()=>{
    const isOn=data.continuousMonitor&&data.continuousMonitor.enabled;
    chrome.runtime.sendMessage({action:"toggleContinuousMonitor",tabId,enabled:!isOn},r=>{
      if(r&&r.ok){
        toast(r.enabled?"Continuous monitor ON":"Continuous monitor OFF");
        setTimeout(load,250);
      }
    });
  });
  // Reference accordion (same pattern as Red mode)
  mh.querySelectorAll("[data-bluref-toggle]").forEach(btn=>{
    btn.addEventListener("click",()=>{
      const sourceId=btn.getAttribute("data-bluref-toggle");
      const wasOpen=btn.getAttribute("aria-expanded")==="true";
      const arrow=btn.querySelector("span");
      const body=btn.nextElementSibling;
      if(!body)return;
...
// Stat tile helper
function blueStatTile(label,n,color){
  return `<div style="background:var(--glass);border:1px solid var(--glassbrd);border-radius:8px;padding:6px 4px;text-align:center">
    <div style="font-family:var(--mono);font-size:14px;font-weight:800;color:${color};line-height:1">${n}</div>
    <div style="font-size:8px;color:var(--t3);text-transform:uppercase;letter-spacing:.6px;font-weight:700;margin-top:2px">${label}</div>
  </div>`;
}

// ---- Finding aggregation. Walks every state field that produces "findings" and
// normalizes them to a common shape: {id, type, severity, where, evidence, raw}.
// id is a stable hash so the same finding across two scans matches itself (used by
// regression diff in Phase 5 and "mark as fixed" persistence).
//
// v6.0.1 — DEDUP DISCIPLINE. The data engine produces one finding per
// (header × response) tuple, so a site with 150 endpoints all missing X-Frame-Options
// generates 150 separate "medium" entries — which under the spec'd score formula
// (`100 - med*3`) instantly drags every site to 0/100. We fix that here:
//   - Header-level issues (missing/CSP/info-leak): main_frame only — these are
//     site-wide config, not per-endpoint. Matches v5.9 Headers tab semantics.
//   - Cookies: dedupe by (cookie name, issue) so one bad cookie counts once even if
//     it appears in every Set-Cookie response.
//   - CORS: dedupe by (path, issue).
//   - Mixed content / Missing SRI: dedupe by host.
//   - Final pass: collapse to one entry per unique (type, where) tuple, keeping the
//     highest severity instance.
function collectBlueFindings(data){
  const raw=[];
  function add(type,severity,where,evidence,extra){
    const id=stableHash([type,where||"",String(evidence||"").substring(0,80)].join("|"));
    raw.push({id,type,severity:severity||"medium",where:where||"",evidence:String(evidence||"").substring(0,300),raw:extra||null});
  }
  // Header-level findings — only the main_frame entry matters. Iterating every
  // response would inflate counts 100x.
  const mainHeader=(data.headers||[]).find(h=>h&&h.type==="main_frame")||(data.headers||[])[0];
  if(mainHeader){
    (mainHeader.missing||[]).forEach(m=>add(`missing-header:${m.header}`,m.severity||"medium","main",m.desc));
    (mainHeader.cspAnalysis?.issues||[]).forEach(i=>add("csp-issue",i.severity||"medium","main",i.desc));
    (mainHeader.leaks||[]).forEach(l=>add(`info-leak:${l.name}`,"info","main",l.value));
  }
  // Cookies — one entry per (cookie name, issue) across the whole response stream.
  ...
  (data.headers||[]).forEach(h=>{
    (h.cookieIssues||[]).forEach(c=>{
      ...
      else add("cookie-issue",c.severity||"medium",c.cookie||"cookie",c.issue);
    });
  });
  // CORS — one entry per (path, issue desc).
  const seenCors=new Set();
  (data.headers||[]).forEach(h=>{
    (h.corsIssues||[]).forEach(c=>{
      const key=`${h.url||""}|${c.desc||""}`;
      if(seenCors.has(key))return;
      seenCors.add(key);
      add("cors-issue",c.severity||"medium",h.url||"main",c.desc);
    });
  });
  // Secrets
  (data.secrets||[]).forEach(s=>add(`exposed-secret:${s.type||""}`,s.severity||"high",s.source||"",s.value));
  // Source map leaks
  (data.sourceMaps||[]).forEach(sm=>add("sourcemap-leak","medium",sm.mapUrl||"",sm.mapUrl));
  // XSS sinks
  (data.xssSinks||[]).forEach(x=>add(`xss-sink:${x.name||""}`,x.severity||"high",x.url||"",x.context));
  // SSTI / XXE / CRLF / proto pollution from probe results
  const pr=data.probeData||{};
  ...
  (pr.protoPollution||[]).forEach(r=>add("proto-pollution",r.severity||"medium",r.path,r.note));
  // BAC + IDOR + auth removal
  (pr.bacResults||[]).forEach(r=>add("broken-access-control",r.severity||"high",r.path,r.note||""));
  (pr.idorAutoResults||[]).forEach(r=>{if(r.sameBody)add("idor-confirmed","high",r.path,"same body across IDs");});
  (pr.authRemovalResults||[]).forEach(r=>{if(r.severity==="critical"||r.severity==="high")add("auth-removal",r.severity,r.path,"unauthenticated access ok");});
  // CORS
  (pr.corsResults||[]).forEach(r=>{if(r.reflected&&r.allowsCredentials)add("cors-wildcard-credentials","high",r.path,"reflected origin + credentials");});
  // CSRF
  ...
  if(pr.graphql&&(pr.graphql.queryFields&&pr.graphql.queryFields.length))add("graphql-introspection","low","/graphql","introspection returned fields");
  // Stack-attack pack hits
  (data.stackAttacks||[]).forEach(a=>{if(a.confirmed)add(`stack-${a.family}-${a.step}`,a.severity||"medium",a.url||"",a.evidence||"");});
  // Mixed content + missing SRI — dedupe by host. A page loading 50 mixed-content
  // resources from cdn.example.com is one fix (configure that CDN to use HTTPS), not
  // 50 fixes. Same logic for missing SRI.
  const seenMixed=new Set();
  (data.mixedContent||[]).forEach(m=>{
    let host="";try{host=new URL(m.url).hostname;}catch(e){host=m.url||"";}
    if(seenMixed.has(host))return;
    seenMixed.add(host);
    add("mixed-content",m.risk||"medium",host,m.type);
  });
  const seenSri=new Set();
  (data.missingSRI||[]).forEach(s=>{
    let host="";try{host=new URL(s.url).hostname;}catch(e){host=s.url||"";}
    if(seenSri.has(host))return;
    seenSri.add(host);
    add("missing-sri","low",host,s.type);
  });

  // v6.0.1 — Final dedup pass. Collapse to one entry per (type, where) tuple, keeping
  // the highest-severity instance. Without this, two scanners that both flag the same
  // missing header (one from CSP analysis, one from missing-headers) would double-count.
  const sevOrd={critical:5,high:4,medium:3,low:2,info:1};
  const dedup=new Map();
  raw.forEach(f=>{
    const key=`${f.type}|${f.where}`;
    const prev=dedup.get(key);
    if(!prev||(sevOrd[f.severity]||0)>(sevOrd[prev.severity]||0)){
      dedup.set(key,f);
    }
  });
  return [...dedup.values()];
...
function countFindingsBySeverity(findings){
  ...
  findings.forEach(f=>{c[f.severity]=(c[f.severity]||0)+1;c.total++;});
  return c;
}

// Health score — STRICT mode (v6.0.1).
//
// Only criticals and highs count. Mediums and lows are still surfaced in the count
// tiles + Top-5 fixes (so users can still triage them), but they do not move the score.
// Rationale: medium-severity findings on a real-world site are mostly missing-header /
// cookie-flag drift that doesn't reflect actual exploitability; counting them dragged
// every site to the floor. Crits and highs are the breach-path findings that matter.
//
// Curve:
...
//
// Weights: critical=22, high=9. Cost capped at 90 so the score floor is 10 — even a
// totally broken site reads as "10/100", not "0/100" (which the user couldn't
// distinguish from "no data yet").
function computeHealthScore(c){
  const cost=Math.min(90,(c.critical||0)*22+(c.high||0)*9);
  return Math.max(10,100-cost);
}

// Prioritize: severity ordinal × ease (so a critical with ease=5 outranks a critical
// with ease=1; among equal severities, easier fixes float up). Returns top N findings
// each enriched with the corresponding FIX_SNIPPETS entry.
function prioritizeFixes(findings,n){
  const sevOrd={critical:5,high:4,medium:3,low:2,info:1};
  const enriched=findings.map(f=>{
    const fix=getFixForFinding(f);
    const ease=fix?fix.ease:2;
    const score=(sevOrd[f.severity]||0)*ease;
    return {finding:f,fix,score};
  }).filter(x=>x.fix);// only items with a known fix
  enriched.sort((a,b)=>b.score-a.score);
  const seen=new Set();
  const out=[];
  // De-duplicate by fix title — multiple findings of same type collapse to one row
...
// Render a single fix row + collapsed body containing snippets
function renderBlueFixRow(item,idx){
  const fix=item.fix;
  const f=item.finding;
  const sevColor=f.severity==="critical"?"var(--red)":f.severity==="high"?"var(--orange)":f.severity==="medium"?"var(--yellow)":f.severity==="low"?"var(--blue)":"var(--t3)";
  const variants=Object.keys(fix).filter(k=>!["title","severity","ease","why","references","snippet_raw"].includes(k));
  const allTabs=["raw",...variants];
  let html=`<div class="blu-fix-row" style="border-top:1px solid var(--glassbrd)">
    <button class="v6-focusable" data-blufix-toggle="${idx}" aria-expanded="false" style="width:100%;background:transparent;border:none;color:var(--t1);font-family:var(--font);padding:9px 14px;display:flex;align-items:center;gap:10px;cursor:pointer;text-align:left">
      <span style="font-family:var(--mono);font-size:11px;color:var(--t3);width:18px">${idx+1}.</span>
      <span style="font-size:10px;font-weight:700;color:${sevColor};padding:2px 6px;border-radius:4px;background:rgba(255,255,255,.05);text-transform:uppercase;letter-spacing:.4px;min-width:54px;text-align:center">${esc(f.severity)}</span>
      <span style="flex:1;font-size:11px;font-weight:600">${esc(fix.title)}</span>
      <span style="font-family:var(--mono);font-size:9px;color:var(--t3)">ease ${fix.ease}/5</span>
      <span style="font-family:var(--mono);font-size:11px;color:var(--t3);width:14px;text-align:right">▸</span>
    </button>
    <div class="blu-fix-body" style="display:none;padding:0 14px 12px;background:var(--bgh)">
      ${fix.why?`<div style="font-size:11px;color:var(--t2);line-height:1.6;padding:8px 0">${esc(fix.why)}</div>`:""}
      <div style="font-size:10px;color:var(--t3);margin:4px 0">Where: <span style="font-family:var(--mono);color:var(--t2)">${esc(f.where||"(general)")}</span></div>
      <div style="display:flex;flex-wrap:wrap;gap:4px;margin:6px 0">`;
  allTabs.forEach((tab,i)=>{
    html+=`<button class="btn v6-focusable ${i===0?"active":""}" data-bluvariant="${escA(tab==="raw"?"snippet_raw":tab)}" data-fixidx="${idx}" style="font-size:9px;padding:3px 8px${i===0?";background:linear-gradient(135deg,var(--purple),var(--pink));color:#fff;border-color:transparent":""}">${esc(tab)}</button>`;
  });
  html+=`</div>
      <pre class="blu-fix-code" style="font-family:var(--mono);font-size:10px;color:var(--green);background:rgba(58,255,138,.04);padding:8px 10px;border-radius:6px;border:1px solid rgba(58,255,138,.15);white-space:pre-wrap;word-break:break-all;max-height:240px;overflow:auto;margin:0">${esc(fix.snippet_raw||"")}</pre>
      <div style="display:flex;gap:6px;margin-top:8px">
        <button class="btn v6-focusable" data-blu-copysnip="${idx}">Copy</button>
        <button class="btn v6-focusable" data-blu-markfix="${escA(f.id)}" style="border-color:rgba(58,255,138,.3);color:var(--green)">Mark as fixed</button>
      </div>`;
  if(fix.references&&fix.references.length){
    html+=`<div style="margin-top:8px;font-size:9px;color:var(--t3)">References:<br>${fix.references.map(r=>`<a href="#" data-copy="${escA(r)}" style="color:var(--blue);text-decoration:none">${esc(r)}</a>`).join("<br>")}</div>`;
  }
  html+=`</div></div>`;
  return html;
}

// ---- FIX_SNIPPETS — inlined from blue-fixes.js so popup.js is self-contained.
// When updating, also update blue-fixes.js. The "why" strings, snippets, and severity
// rationale are identical between the two files.
const FIX_SNIPPETS={
  "missing-hsts":{title:"Add Strict-Transport-Security header",severity:"high",ease:5,why:"Prevents protocol downgrade attacks. Forces HTTPS for the configured max-age. Combined with `preload` submission, also protects first-time visitors.",snippet_raw:"Strict-Transport-Security: max-age=63072000; includeSubDomains; preload",nginx:'add_header Strict-Transport-Security "max-age=63072000; includeSubDomains; preload" always;',apache:'Header always set Strict-Transport-Security "max-age=63072000; includeSubDomains; preload"',iis:'<add name="Strict-Transport-Security" value="max-age=63072000; includeSubDomains; preload" />',cloudflare:"Cloudflare → SSL/TLS → Edge Certificates → HTTP Strict Transport Security: enable with max-age=2y, includeSubDomains, preload.",express:"app.use((req,res,next)=>{ res.setHeader('Strict-Transport-Security','max-age=63072000; includeSubDomains; preload'); next(); });",references:["https://hstspreload.org/"]},
  "missing-csp":{title:"Add Content-Security-Policy header",severity:"high",ease:3,why:"CSP is the strongest defense against XSS. Use Blue mode's CSP Generator to produce one from observed traffic.",snippet_raw:"Content-Security-Policy: default-src 'none'; script-src 'self'; style-src 'self'; img-src 'self' data:; connect-src 'self'; font-src 'self'; frame-ancestors 'none'; base-uri 'self'; form-action 'self'; upgrade-insecure-requests",nginx:`add_header Content-Security-Policy "default-src 'none'; script-src 'self'; style-src 'self'; img-src 'self' data:; connect-src 'self'; font-src 'self'; frame-ancestors 'none'; base-uri 'self'; form-action 'self'; upgrade-insecure-requests" always;`,apache:`Header always set Content-Security-Policy "default-src 'none'; script-src 'self'; style-src 'self'; img-src 'self' data:; connect-src 'self'; font-src 'self'; frame-ancestors 'none'; base-uri 'self'; form-action 'self'; upgrade-insecure-requests"`,express:"const helmet=require('helmet'); app.use(helmet.contentSecurityPolicy({ directives:{ defaultSrc:[\"'none'\"], scriptSrc:[\"'self'\"], styleSrc:[\"'self'\"], imgSrc:[\"'self'\",\"data:\"], connectSrc:[\"'self'\"], fontSrc:[\"'self'\"], frameAncestors:[\"'none'\"], baseUri:[\"'self'\"], formAction:[\"'self'\"] } }));",references:["https://content-security-policy.com/"]},
  "missing-xframe":{title:"Add X-Frame-Options or CSP frame-ancestors",severity:"medium",ease:5,why:"Prevents clickjacking. CSP frame-ancestors supersedes X-Frame-Options where supported.",snippet_raw:"X-Frame-Options: DENY",nginx:'add_header X-Frame-Options "DENY" always;',apache:'Header always set X-Frame-Options "DENY"',express:"res.setHeader('X-Frame-Options','DENY');"},
  "missing-xcto":{title:"Add X-Content-Type-Options: nosniff",severity:"low",ease:5,why:"Prevents MIME sniffing.",snippet_raw:"X-Content-Type-Options: nosniff",nginx:'add_header X-Content-Type-Options "nosniff" always;',apache:'Header always set X-Content-Type-Options "nosniff"',express:"res.setHeader('X-Content-Type-Options','nosniff');"},
  "missing-referrer":{title:"Add Referrer-Policy header",severity:"low",ease:5,why:"Controls referrer leakage. strict-origin-when-cross-origin is the modern default.",snippet_raw:"Referrer-Policy: strict-origin-when-cross-origin",nginx:'add_header Referrer-Policy "strict-origin-when-cross-origin" always;'},
  "missing-permissions-policy":{title:"Add Permissions-Policy header",severity:"low",ease:4,why:"Disable unused browser APIs (camera, microphone, geolocation) so XSS can't pivot into them.",snippet_raw:"Permissions-Policy: camera=(), microphone=(), geolocation=(), payment=(), usb=()",nginx:'add_header Permissions-Policy "camera=(), microphone=(), geolocation=(), payment=(), usb=()" always;'},
  "missing-coop":{title:"Add Cross-Origin-Opener-Policy header",severity:"low",ease:4,why:"Isolates the browsing context — protects against XS-Leaks and Spectre-class attacks.",snippet_raw:"Cross-Origin-Opener-Policy: same-origin",nginx:'add_header Cross-Origin-Opener-Policy "same-origin" always;'},
  "cookie-no-httponly":{title:"Add HttpOnly to session cookie",severity:"medium",ease:5,why:"Prevents document.cookie XSS reads of the session.",snippet_raw:"Set-Cookie: session=...; HttpOnly; Secure; SameSite=Strict",express:"res.cookie('session', token, { httpOnly:true, secure:true, sameSite:'strict' });",django:"SESSION_COOKIE_HTTPONLY = True\nSESSION_COOKIE_SECURE = True\nSESSION_COOKIE_SAMESITE = 'Strict'",laravel:"// config/session.php\n'http_only'=>true, 'secure'=>true, 'same_site'=>'strict',",rails:"# config/initializers/session_store.rb\nRails.application.config.session_store :cookie_store, key:'_app_session', httponly:true, secure:Rails.env.production?, same_site::strict",aspnet:"options.Cookie.HttpOnly = true;\noptions.Cookie.SecurePolicy = CookieSecurePolicy.Always;\noptions.Cookie.SameSite = SameSiteMode.Strict;"},
  "cookie-no-secure":{title:"Add Secure flag to cookie",severity:"medium",ease:5,why:"Without Secure the cookie is sent over plaintext HTTP — any network attacker can read it.",snippet_raw:"Set-Cookie: session=...; Secure; HttpOnly; SameSite=Strict",express:"res.cookie('session', token, { secure:true });"},
  "cookie-no-samesite":{title:"Set SameSite on cookie",severity:"medium",ease:5,why:"SameSite=Strict blocks CSRF on most flows.",snippet_raw:"Set-Cookie: session=...; SameSite=Strict; Secure; HttpOnly",express:"res.cookie('session', token, { sameSite:'strict' });"},
  "sourcemap-leak":{title:"Remove .map files from production",severity:"medium",ease:5,why:"Source maps deobfuscate code, exposing original variable names and sometimes secrets.",snippet_raw:"# Webpack:\nmodule.exports={ devtool:false }\n# Vite:\nbuild:{ sourcemap:false }",nginx:"location ~* \\.map$ { deny all; return 404; }",apache:'<FilesMatch "\\.map$"> Require all denied </FilesMatch>'},
  "exposed-secret":{title:"Rotate the exposed secret",severity:"critical",ease:2,why:"Once a secret is committed/served, assume it's compromised. Rotate immediately and use a secrets manager.",snippet_raw:"# 1. Rotate upstream\n# 2. Remove from code: git rm <file>\n# 3. Purge history: git filter-repo --path <file> --invert-paths\n# 4. Force-push, notify team\n# 5. Inject from secrets manager at runtime"},
  "sql-injection-confirmed":{title:"Use parameterized queries",severity:"critical",ease:3,why:"Never concatenate user input into SQL. Prepared statements / ORMs prevent injection.",snippet_raw:"// BAD\ndb.query('SELECT * FROM users WHERE id='+id)\n// GOOD\ndb.query('SELECT * FROM users WHERE id=?',[id])",express:"const [rows]=await pool.query('SELECT * FROM users WHERE id=?',[req.params.id]);",django:"User.objects.get(id=request.GET.get('id'))",laravel:"DB::select('SELECT * FROM users WHERE id=?',[$id]);"},
  "ssti-confirmed":{title:"Don't pass user input into template engine context",severity:"critical",ease:2,why:"SSTI = RCE in the template engine. Render user input as data via {{ }}, never via render_template_string.",snippet_raw:"# BAD (Flask)\nreturn render_template_string('Hello '+name)\n# GOOD\nreturn render_template('hello.html', name=name)"},
  "xxe-confirmed":{title:"Disable external entities in XML parser",severity:"critical",ease:4,why:"XXE allows file disclosure, SSRF, sometimes RCE. Disable DOCTYPE and external entities.",snippet_raw:"// Java\ndbf.setFeature('http://apache.org/xml/features/disallow-doctype-decl',true);\n// Python (lxml)\nparser=etree.XMLParser(resolve_entities=False, no_network=True)\n// PHP\nlibxml_disable_entity_loader(true);"},
  "crlf-injection":{title:"Strip \\r and \\n from header values",severity:"high",ease:5,why:"CRLF injection enables response splitting and cache poisoning.",snippet_raw:"const safe=String(input).replace(/[\\r\\n]/g,'');\nres.setHeader('Location',safe);"},
  "open-redirect":{title:"Validate redirect targets against an allowlist",severity:"medium",ease:4,why:"Open redirects power phishing and OAuth code stealing.",snippet_raw:"const ALLOW=['/dashboard','/profile','/settings'];\nfunction safe(t){ return ALLOW.includes(t)?t:'/'; }"},
  "cors-wildcard-credentials":{title:"Replace wildcard CORS with explicit allowlist",severity:"high",ease:4,why:"Reflecting Origin + Allow-Credentials breaks SOP entirely.",snippet_raw:"const ALLOW=new Set(['https://app.example.com']);\napp.use((req,res,next)=>{ const o=req.headers.origin; if(ALLOW.has(o)){ res.setHeader('Access-Control-Allow-Origin',o); res.setHeader('Access-Control-Allow-Credentials','true'); } next(); });"},
  "missing-csrf":{title:"Add CSRF tokens to state-changing forms",severity:"medium",ease:4,why:"Without tokens, attackers can trigger logged-in users' actions from another site.",snippet_raw:"const csurf=require('csurf');\napp.use(csurf({ cookie:true }));",django:"# Django enables CSRF by default — keep {% csrf_token %} in every <form>",laravel:"<!-- Blade -->\n@csrf"},
  "jwt-alg-none":{title:"Reject JWT alg=none",severity:"critical",ease:4,why:"Pin the algorithm explicitly when verifying. Never let the JWT itself dictate it.",snippet_raw:"jwt.verify(token, secret, { algorithms:['HS256'] }); // Node\njwt.decode(token, secret, algorithms=['HS256']) # Python"},
  "graphql-introspection":{title:"Disable GraphQL introspection in production",severity:"low",ease:5,why:"Introspection lets anyone map your full schema.",snippet_raw:"new ApolloServer({ introspection: process.env.NODE_ENV !== 'production' })"},
  "exposed-debug":{title:"Disable debug/trace/admin endpoints in production",severity:"high",ease:4,why:"/debug, /actuator, /trace.axd, /telescope leak internals or RCE surface.",snippet_raw:"# Spring Boot\nmanagement.endpoints.web.exposure.include=health,info\n# Laravel: remove /telescope, /horizon middleware\n# ASP.NET: remove trace.axd, elmah.axd"},
  "directory-listing":{title:"Disable directory listing",severity:"medium",ease:5,why:"Indexes expose backup files, .git, .env.",snippet_raw:"# Apache\nOptions -Indexes\n# Nginx\nautoindex off;"},
  "weak-tls":{title:"Disable TLS 1.0/1.1 and weak ciphers",severity:"high",ease:3,why:"TLS 1.0/1.1 are deprecated. Modern AEAD ciphers + forward secrecy.",snippet_raw:"# Nginx\nssl_protocols TLSv1.2 TLSv1.3;\nssl_ciphers ECDHE-ECDSA-AES256-GCM-SHA384:ECDHE-RSA-AES256-GCM-SHA384;\nssl_prefer_server_ciphers off;"},
  "mixed-content":{title:"Upgrade subresources to HTTPS",severity:"medium",ease:4,why:"HTTPS pages loading HTTP resources are MITM-able.",snippet_raw:"Content-Security-Policy: upgrade-insecure-requests"},
  "missing-sri":{title:"Add Subresource Integrity to CDN scripts",severity:"low",ease:4,why:"If the CDN is compromised, SRI prevents tainted code from running.",snippet_raw:'<script src="https://cdn.example.com/lib.js" integrity="sha384-..." crossorigin="anonymous"></script>'},
  "verbose-error":{title:"Hide stack traces in production",severity:"medium",ease:4,why:"Stack traces leak file paths, framework versions, sometimes user data.",snippet_raw:"app.use((err,req,res,next)=>{ console.error(err); res.status(500).json({ error:'Internal error' }); });"},
  "exposed-env":{title:"Block access to /.env, /.git, dotfiles",severity:"critical",ease:5,why:"Dotfiles leak DB credentials, JWT secrets, full source trees.",snippet_raw:"# Nginx\nlocation ~ /\\. { deny all; return 404; }\n# Apache\nRedirectMatch 404 /\\..*$"},
  "broken-access-control":{title:"Enforce server-side authorization on every endpoint",severity:"critical",ease:3,why:"Hidden UI buttons are not security. Every endpoint must check the caller's role.",snippet_raw:"function requireRole(role){ return (req,res,next)=>{ if(req.user?.role!==role) return res.status(403).end(); next(); }; }\napp.delete('/api/users/:id', requireRole('admin'), handler);"},
  "idor-confirmed":{title:"Verify resource ownership before returning data",severity:"high",ease:3,why:"Always scope queries by authenticated user, not just by ID.",snippet_raw:"// BAD\nconst order=await Order.findById(req.params.id);\n// GOOD\nconst order=await Order.findOne({ _id:req.params.id, userId:req.user.id });"},
  "rate-limit":{title:"Add rate limiting to auth endpoints",severity:"medium",ease:4,why:"Without limits, password and OTP endpoints are wide open to brute force and enumeration.",snippet_raw:"const { RateLimiterMemory }=require('rate-limiter-flexible');\nconst limiter=new RateLimiterMemory({ points:5, duration:60 });\napp.post('/login', async (req,res)=>{ try{ await limiter.consume(req.ip); }catch(e){ return res.status(429).end(); } /* auth */ });"},
};

function mapFindingToFixKey(f){
  if(!f)return null;
  const t=String(f.type||f.id||"").toLowerCase();
  if(/strict-transport|hsts/.test(t))return "missing-hsts";
  if(/content-security-policy|^csp/.test(t)||/csp/.test(t))return "missing-csp";
  if(/x-frame|clickjack/.test(t))return "missing-xframe";
  if(/x-content-type|nosniff/.test(t))return "missing-xcto";
  if(/referrer/.test(t))return "missing-referrer";
  if(/permissions-policy|feature-policy/.test(t))return "missing-permissions-policy";
  if(/coop|cross-origin-opener/.test(t))return "missing-coop";
  if(/httponly/.test(t))return "cookie-no-httponly";
  if(/samesite/.test(t))return "cookie-no-samesite";
  if(/cookie.*secure|cookie-no-secure/.test(t))return "cookie-no-secure";
  if(/source ?map|sourcemap/.test(t))return "sourcemap-leak";
  ...
  if(/weak.*tls|cipher|ssl/.test(t))return "weak-tls";
  ...
  if(/bac\b|broken access|broken-access/.test(t))return "broken-access-control";
  if(/rate.*limit|brute/.test(t))return "rate-limit";
  if(/secret|api ?key|token|password|credential/.test(t))return "exposed-secret";
  return null;
}
function getFixForFinding(f){const k=mapFindingToFixKey(f);return k?FIX_SNIPPETS[k]:null;}

// Stable hash for finding IDs (regression diff). Tiny FNV-1a 32-bit — collision risk is
// acceptable for the size of finding sets we deal with (typically < 200 per scan).
...
// ---- Action panels: CSP, compliance, diff. Mount into #bluPanels (cleared first).
function showCSPPanel(data){
  const wrap=document.getElementById("bluPanels");
  if(!wrap)return;
  const oldCsp=findExistingCsp(data);
  const result=generateTightCSP(data,oldCsp);
  let html=`<div style="padding:14px 16px;border-top:1px solid var(--glassbrd);background:var(--bgh)">
    <div style="font-size:11px;color:var(--purple);font-weight:700;text-transform:uppercase;letter-spacing:.5px;margin-bottom:8px">Generated CSP (from observed traffic)</div>
    <pre style="font-family:var(--mono);font-size:10px;color:var(--green);background:rgba(58,255,138,.04);padding:8px 10px;border-radius:6px;border:1px solid rgba(58,255,138,.15);white-space:pre-wrap;word-break:break-all;cursor:pointer;margin:0" data-copytext="1">${esc(result.header)}</pre>`;
  if(result.warnings&&result.warnings.length){
    html+=`<div style="margin-top:10px;padding:8px 10px;border-radius:6px;background:rgba(255,197,58,.08);border:1px solid rgba(255,197,58,.3);font-size:10px;color:var(--yellow)">⚠ ${result.warnings.map(esc).join("<br>⚠ ")}</div>`;
  }
  // Per-directive breakdown
  html+=`<div style="margin-top:10px;font-size:10px;color:var(--t3)">Per-directive sources (${Object.keys(result.directives).length} directives):</div><div style="margin-top:4px">`;
  Object.entries(result.directives).forEach(([dir,srcs])=>{
    html+=`<div style="font-family:var(--mono);font-size:10px;padding:3px 0"><span style="color:var(--purple)">${esc(dir)}</span> <span style="color:var(--t2)">${esc(srcs.join(" "))}</span></div>`;
  });
  html+=`</div>`;
  // Diff
  if(result.diff){
    html+=`<div style="margin-top:10px;font-size:10px;color:var(--t3)">Diff vs current CSP:</div>`;
    if(result.diff.tightened.length){html+=`<div style="color:var(--green);font-size:10px">+ Tightened: ${result.diff.tightened.map(d=>esc(d.directive)).join(", ")}</div>`;}
    if(result.diff.loosened.length){html+=`<div style="color:var(--red);font-size:10px">- Loosened: ${result.diff.loosened.map(d=>esc(d.directive)).join(", ")}</div>`;}
    if(result.diff.added.length){html+=`<div style="color:var(--yellow);font-size:10px">+ Added directives: ${result.diff.added.map(d=>esc(d.directive)).join(", ")}</div>`;}
    if(result.diff.same.length){html+=`<div style="color:var(--t3);font-size:10px">= Unchanged: ${result.diff.same.map(d=>esc(d.directive)).join(", ")}</div>`;}
  }
  html+=`<div style="display:flex;gap:6px;margin-top:10px">
    <button class="btn v6-focusable" id="bluCspCopy">Copy header</button>
    <button class="btn v6-focusable" id="bluCspClose">Close</button>
  </div></div>`;
  wrap.innerHTML=html;
  document.getElementById("bluCspCopy")?.addEventListener("click",()=>{
    navigator.clipboard.writeText(result.header).then(()=>toast("Copied CSP"));
  });
  document.getElementById("bluCspClose")?.addEventListener("click",()=>{wrap.innerHTML="";});
}

// Find existing CSP value from any observed header for diff comparison
function findExistingCsp(data){
  for(const h of (data.headers||[])){
    if(h.cspAnalysis&&h.cspAnalysis.value)return h.cspAnalysis.value;
    if(h.value&&typeof h.value==="string"&&/script-src|default-src/i.test(h.value))return h.value;
  }
  return null;
}

// Inlined generateTightCSP — see blue-csp.js for the canonical / commented version
function generateTightCSP(tabData,oldCsp){
  let origin="";try{if(tabData&&tabData.url)origin=new URL(tabData.url).origin;}catch(e){}
  function srcOf(url){try{const u=new URL(url);if(origin&&u.origin===origin)return "'self'";return u.origin;}catch(e){return null;}}
  const D={"default-src":new Set(["'none'"]),"script-src":new Set(),"style-src":new Set(),"img-src":new Set(),"font-src":new Set(),"connect-src":new Set(),"frame-src":new Set(),"media-src":new Set(),"manifest-src":new Set(),"worker-src":new Set(),"frame-ancestors":new Set(["'none'"]),"base-uri":new Set(["'self'"]),"form-action":new Set(["'self'"])};
  const warnings=[];
  const TYPE_MAP={"script":["script-src"],"stylesheet":["style-src"],"image":["img-src"],"font":["font-src"],"xhr":["connect-src"],"fetch":["connect-src"],"websocket":["connect-src"],"iframe":["frame-src"],"media":["media-src"],"manifest":["manifest-src"],"worker":["worker-src","script-src"]};
  (tabData.endpoints||[]).forEach(e=>{if(!e||!e.url)return;const dirs=TYPE_MAP[String(e.type||"").toLowerCase()];if(!dirs)return;const s=srcOf(e.url);if(!s)return;dirs.forEach(d=>D[d].add(s));});
  (tabData.iframeScan||[]).forEach(i=>{if(!i||!i.url)return;const s=srcOf(i.url);if(s)D["frame-src"].add(s);});
  (tabData.wsConnections||[]).forEach(w=>{if(!w||!w.url)return;try{const u=new URL(w.url);D["connect-src"].add(u.origin);}catch(e){}});
  const ss=tabData.scriptSources||[];
  if(ss.some(s=>s&&s.scriptUrl&&/^inline/i.test(s.scriptUrl))){D["script-src"].add("'unsafe-inline'");warnings.push("Inline <script> tags detected — kept 'unsafe-inline'. Migrate to nonces/hashes for tighter policy.");}
  if(ss.some(s=>s&&s.pattern&&/eval|new Function/i.test(s.pattern))){D["script-src"].add("'unsafe-eval'");warnings.push("eval() usage detected — kept 'unsafe-eval'.");}
  ["script-src","style-src","img-src","font-src","connect-src"].forEach(d=>{if(D[d].size===0)D[d].add("'self'");});
  ...
  const parts=[];order.forEach(d=>{if(D[d].size===0)return;parts.push(d+" "+[...D[d]].join(" "));});
  ...
  if(oldCsp&&typeof oldCsp==="string"){
    ...
    Object.keys(D).forEach(d=>{
      ...
      else diff.tightened.push({directive:d,from:oldSet,to:newSet});
    });
  }
  return {header,directives:Object.fromEntries(Object.entries(D).map(([k,v])=>[k,[...v]])),warnings,diff};
}
function parseCspString(csp){const out={};if(!csp||typeof csp!=="string")return out;csp.split(";").forEach(part=>{const p=part.trim();if(!p)return;const tokens=p.split(/\s+/);const dir=tokens.shift().toLowerCase();if(!dir)return;out[dir]=new Set(tokens);});return out;}

// Snapshot reader for trend arrow on health score header
function loadLastSnapshotMeta(url){
  try{
    let host="";try{host=new URL(url||"").hostname;}catch(e){return null;}
    if(!host)return null;
    // chrome.storage.local is async — for synchronous trend display we cache the last
    // known meta in window-local memory. The first popup open returns no trend; the
    // second returns one. The Phase 5 message handler also returns the snapshot list.
    return _bluLastSnapMeta&&_bluLastSnapMeta.host===host?_bluLastSnapMeta:null;
  }catch(e){return null;}
}
let _bluLastSnapMeta=null;

// Compliance + diff panels — Phase 6 fills compliance, Phase 5 fills diff. Stub the
// shells now so the buttons are visibly responsive in Phase 4 builds.
function showCompliancePanel(data,findings){
  const wrap=document.getElementById("bluPanels");
  if(!wrap)return;
  // Tag every finding with its fix key so compliance lookup works
  const tagged=findings.map(f=>({...f,_fixKey:mapFindingToFixKey(f)}));
  // For each framework, count: total controls cited vs. count where we have a clean
  // bill of health (i.e. no findings violate a known control). Coverage % is a rough
  // proxy — it's the share of frameworks for which PenScope found zero violations.
  const perFramework={};
  COMPLIANCE_FRAMEWORKS.forEach(fw=>{perFramework[fw.key]={fw,violations:[],coverage:100};});
  tagged.forEach(f=>{
    const ctrls=getComplianceForFinding(f);
    ctrls.forEach(c=>{
      perFramework[c.framework]?.violations.push({finding:f,control:c});
    });
  });
  // Coverage = 100% minus min(violations*4, 80). A 25-violation framework caps at 20%.
  Object.values(perFramework).forEach(p=>{p.coverage=Math.max(20,100-Math.min(p.violations.length*4,80));});

  let html=`<div id="bluCompliancePanel" style="padding:14px 16px;border-top:1px solid var(--glassbrd);background:var(--bgh)">
    <div style="font-size:11px;color:var(--purple);font-weight:700;text-transform:uppercase;letter-spacing:.5px;margin-bottom:8px">Compliance coverage — ${tagged.length} findings mapped</div>`;
  COMPLIANCE_FRAMEWORKS.forEach(fw=>{
    const p=perFramework[fw.key];
    const barW=Math.round(p.coverage);
    html+=`<div style="margin:8px 0">
      <div style="display:flex;align-items:center;gap:8px;font-size:11px">
        <span style="font-weight:700;color:${fw.color};min-width:120px">${esc(fw.label)}</span>
        <div style="flex:1;height:6px;background:var(--glass2);border-radius:3px;overflow:hidden"><div style="width:${barW}%;height:100%;background:${fw.color}"></div></div>
        <span style="font-family:var(--mono);font-size:10px;color:var(--t2);min-width:48px;text-align:right">${barW}%</span>
        <span style="font-family:var(--mono);font-size:9px;color:var(--t3);min-width:60px;text-align:right">${p.violations.length} hits</span>
      </div>`;
    if(p.violations.length){
      html+=`<div style="margin-top:4px;padding-left:8px">`;
      p.violations.slice(0,8).forEach(v=>{
        html+=`<div style="font-family:var(--mono);font-size:9px;color:var(--t3);padding:1px 0">✗ <span style="color:var(--t2)">${esc(v.control.control)}</span> — ${esc(v.control.desc)} <span style="color:var(--t3)">(${esc(v.finding.type)} on ${esc(v.finding.where||"main")})</span></div>`;
      });
      if(p.violations.length>8)html+=`<div style="font-size:9px;color:var(--t3)">…${p.violations.length-8} more</div>`;
      html+=`</div>`;
    }
    html+=`</div>`;
  });
  html+=`<div style="display:flex;gap:6px;margin-top:14px">
    <button class="btn v6-focusable" id="bluComplPdf">Export PDF</button>
    <button class="btn v6-focusable" id="bluComplJson">Export JSON</button>
    <button class="btn v6-focusable" id="bluComplClose">Close</button>
  </div></div>`;
  wrap.innerHTML=html;

  document.getElementById("bluComplJson")?.addEventListener("click",()=>{
    const payload={
      timestamp:new Date().toISOString(),
      hostname:(()=>{try{return new URL(data.url||"").hostname;}catch(e){return "";}})(),
      penscope_version:"6.0.0",
      total_findings:tagged.length,
      frameworks:Object.values(perFramework).map(p=>({
        framework:p.fw.key,label:p.fw.label,coverage_percent:p.coverage,
        violations:p.violations.map(v=>({control:v.control.control,desc:v.control.desc,finding_type:v.finding.type,severity:v.finding.severity,where:v.finding.where,evidence:v.finding.evidence}))
      }))
    };
    const blob=new Blob([JSON.stringify(payload,null,2)],{type:"application/json"});
    const url=URL.createObjectURL(blob);
    const a=document.createElement("a");a.href=url;a.download=`penscope_compliance_${(payload.hostname||"target").replace(/[^a-z0-9]/gi,"_")}.json`;a.click();
    setTimeout(()=>URL.revokeObjectURL(url),1000);
    toast("JSON exported");
  });

  document.getElementById("bluComplPdf")?.addEventListener("click",()=>{
    // PDF export uses window.print() with a print-only style. We open a new tab with
    // a print-friendly HTML rendering of the panel and trigger print there to keep the
    // popup chrome out of the output.
    const css=`
      @page { size: A4; margin: 14mm; }
      body { font-family: 'Plus Jakarta Sans', Arial, sans-serif; color:#222; font-size:11pt; }
      h1 { font-size:18pt; color:#9b5aff; margin-bottom:4mm; }
      h2 { font-size:13pt; color:#333; border-bottom:1px solid #999; padding-bottom:1mm; margin-top:6mm; }
      .meta { font-size:9pt; color:#666; margin-bottom:6mm; }
      .fw { margin-bottom:5mm; }
      .fw-h { font-weight:700; color:#9b5aff; }
      .bar { height:4mm; background:#eee; border-radius:1mm; overflow:hidden; margin:1mm 0; }
      .bar > div { height:100%; background:#3aa8ff; }
      table { border-collapse:collapse; width:100%; font-size:9pt; }
      th, td { border:1px solid #ccc; padding:1mm 2mm; text-align:left; vertical-align:top; }
      th { background:#f5f5f5; }
    `;
    let body=`<h1>PenScope Compliance Audit</h1><div class="meta">Generated ${new Date().toLocaleString()} · Target: ${esc((()=>{try{return new URL(data.url||"").hostname;}catch(e){return "(unknown)";}})())} · ${tagged.length} findings mapped · PenScope v6.0</div>`;
    COMPLIANCE_FRAMEWORKS.forEach(fw=>{
      const p=perFramework[fw.key];
      body+=`<div class="fw"><div class="fw-h">${esc(fw.label)} — ${p.coverage}% coverage (${p.violations.length} violations)</div><div class="bar"><div style="width:${p.coverage}%;background:${fw.color}"></div></div>`;
      if(p.violations.length){
        body+=`<table><thead><tr><th>Control</th><th>Description</th><th>Finding</th><th>Severity</th><th>Where</th></tr></thead><tbody>`;
        p.violations.forEach(v=>{
          body+=`<tr><td>${esc(v.control.control)}</td><td>${esc(v.control.desc)}</td><td>${esc(v.finding.type)}</td><td>${esc(v.finding.severity)}</td><td>${esc(v.finding.where||"")}</td></tr>`;
        });
        body+=`</tbody></table>`;
      }
      body+=`</div>`;
    });
    const html=`<!doctype html><html><head><meta charset="utf-8"><title>PenScope Compliance — ${esc((()=>{try{return new URL(data.url||"").hostname;}catch(e){return "report";}})())}</title><style>${css}</style></head><body>${body}<script>window.print();</script></body></html>`;
    const w=window.open("","_blank","noopener");
    if(w){w.document.open();w.document.write(html);w.document.close();}
    else toast("Popup blocked — allow popups for PenScope");
  });

  document.getElementById("bluComplClose")?.addEventListener("click",()=>{wrap.innerHTML="";});
}

// ---- COMPLIANCE_MAP — inlined from blue-compliance.js. Update both when changing.
...
const COMPLIANCE_MAP={
  "missing-hsts":[{framework:"PCI-DSS-v4",control:"4.2.1",desc:"Strong cryptography for data in transit"},{framework:"NESA-UAE-IAS",control:"T2.3.5",desc:"Secure communications"},{framework:"SAMA-CSF",control:"3.3.14",desc:"Cryptography"},{framework:"DESC-ISR",control:"CO.2.1",desc:"Cryptographic protection"},{framework:"ISO-27001",control:"A.13.2.1",desc:"Information transfer policies"},{framework:"OWASP-Top10",control:"A02:2021",desc:"Cryptographic Failures"},{framework:"CWE",control:"CWE-319",desc:"Cleartext Transmission of Sensitive Information"}],
  "missing-csp":[{framework:"PCI-DSS-v4",control:"6.4.2",desc:"Web application protection"},{framework:"NESA-UAE-IAS",control:"T7.4.3",desc:"Web application security"},{framework:"SAMA-CSF",control:"3.3.5",desc:"Application security"},{framework:"ISO-27001",control:"A.14.2.5",desc:"Secure system engineering"},{framework:"OWASP-Top10",control:"A05:2021",desc:"Security Misconfiguration"},{framework:"CWE",control:"CWE-693",desc:"Protection Mechanism Failure"}],
  "missing-xframe":[{framework:"OWASP-Top10",control:"A05:2021",desc:"Security Misconfiguration"},{framework:"CWE",control:"CWE-1021",desc:"Improper Restriction of Rendered UI Layers"}],
  "missing-xcto":[{framework:"OWASP-Top10",control:"A05:2021",desc:"Security Misconfiguration"},{framework:"CWE",control:"CWE-430",desc:"Deployment of Wrong Handler"}],
  "missing-referrer":[{framework:"OWASP-Top10",control:"A05:2021",desc:"Security Misconfiguration"},{framework:"CWE",control:"CWE-200",desc:"Exposure of Sensitive Information"}],
  "missing-permissions-policy":[{framework:"OWASP-Top10",control:"A05:2021",desc:"Security Misconfiguration"}],
  "missing-coop":[{framework:"OWASP-Top10",control:"A05:2021",desc:"Security Misconfiguration"}],
  "cookie-no-httponly":[{framework:"PCI-DSS-v4",control:"6.2.4",desc:"Software protected against attacks"},{framework:"OWASP-Top10",control:"A05:2021",desc:"Security Misconfiguration"},{framework:"ISO-27001",control:"A.14.1.3",desc:"Protecting application services"},{framework:"CWE",control:"CWE-1004",desc:"Sensitive Cookie Without HttpOnly Flag"}],
  "cookie-no-secure":[{framework:"PCI-DSS-v4",control:"4.2.1",desc:"Strong cryptography in transit"},{framework:"OWASP-Top10",control:"A02:2021",desc:"Cryptographic Failures"},{framework:"CWE",control:"CWE-614",desc:"Sensitive Cookie Without Secure"}],
  "cookie-no-samesite":[{framework:"PCI-DSS-v4",control:"6.4.2",desc:"Web application protection"},{framework:"OWASP-Top10",control:"A01:2021",desc:"Broken Access Control"},{framework:"CWE",control:"CWE-1275",desc:"Sensitive Cookie with Improper SameSite"}],
  "sourcemap-leak":[{framework:"PCI-DSS-v4",control:"3.5.1",desc:"Cryptographic keys protection"},{framework:"NESA-UAE-IAS",control:"T6.1.7",desc:"Information leakage prevention"},{framework:"SAMA-CSF",control:"3.3.5",desc:"Application security"},{framework:"ISO-27001",control:"A.14.2.7",desc:"Outsourced development"},{framework:"OWASP-Top10",control:"A05:2021",desc:"Security Misconfiguration"},{framework:"CWE",control:"CWE-540",desc:"Inclusion of Sensitive Info in Source Code"}],
  "exposed-secret":[{framework:"PCI-DSS-v4",control:"6.5.1",desc:"Application secrets protection"},{framework:"NESA-UAE-IAS",control:"T1.2.4",desc:"Credential management"},{framework:"SAMA-CSF",control:"3.3.10",desc:"Identity and access management"},{framework:"DESC-ISR",control:"AM.2.4",desc:"Privileged access controls"},{framework:"ISO-27001",control:"A.9.4.3",desc:"Password management"},{framework:"OWASP-Top10",control:"A07:2021",desc:"Identification and Authentication Failures"},{framework:"CWE",control:"CWE-798",desc:"Use of Hard-coded Credentials"}],
  "sql-injection-confirmed":[{framework:"PCI-DSS-v4",control:"6.2.4",desc:"Software protected against attacks"},{framework:"NESA-UAE-IAS",control:"T6.1.2",desc:"Input validation"},{framework:"SAMA-CSF",control:"3.3.5",desc:"Application security"},{framework:"DESC-ISR",control:"AC.1.2",desc:"Application controls"},{framework:"ISO-27001",control:"A.14.2.5",desc:"Secure system engineering"},{framework:"OWASP-Top10",control:"A03:2021",desc:"Injection"},{framework:"CWE",control:"CWE-89",desc:"SQL Injection"}],
  "ssti-confirmed":[{framework:"OWASP-Top10",control:"A03:2021",desc:"Injection"},{framework:"PCI-DSS-v4",control:"6.2.4",desc:"Software protected against attacks"},{framework:"NESA-UAE-IAS",control:"T6.1.2",desc:"Input validation"},{framework:"CWE",control:"CWE-1336",desc:"Template Injection"}],
  "xxe-confirmed":[{framework:"OWASP-Top10",control:"A05:2021",desc:"Security Misconfiguration"},{framework:"NESA-UAE-IAS",control:"T6.1.2",desc:"Input validation"},{framework:"CWE",control:"CWE-611",desc:"XML External Entity Reference"}],
  "crlf-injection":[{framework:"OWASP-Top10",control:"A03:2021",desc:"Injection"},{framework:"CWE",control:"CWE-93",desc:"CRLF Injection"}],
  "open-redirect":[{framework:"OWASP-Top10",control:"A01:2021",desc:"Broken Access Control"},{framework:"NESA-UAE-IAS",control:"T6.1.5",desc:"Output encoding / redirect validation"},{framework:"CWE",control:"CWE-601",desc:"URL Redirection to Untrusted Site"}],
  "cors-wildcard-credentials":[{framework:"OWASP-Top10",control:"A05:2021",desc:"Security Misconfiguration"},{framework:"PCI-DSS-v4",control:"6.4.2",desc:"Web application protection"},{framework:"NESA-UAE-IAS",control:"T7.4.3",desc:"Web application security"},{framework:"CWE",control:"CWE-942",desc:"Permissive Cross-domain Policy"}],
  "missing-csrf":[{framework:"OWASP-Top10",control:"A01:2021",desc:"Broken Access Control"},{framework:"PCI-DSS-v4",control:"6.2.4",desc:"Software protected against attacks"},{framework:"CWE",control:"CWE-352",desc:"Cross-Site Request Forgery"}],
  "jwt-alg-none":[{framework:"OWASP-Top10",control:"A02:2021",desc:"Cryptographic Failures"},{framework:"PCI-DSS-v4",control:"4.2.1",desc:"Strong cryptography"},{framework:"SAMA-CSF",control:"3.3.14",desc:"Cryptography"},{framework:"CWE",control:"CWE-347",desc:"Improper Verification of Cryptographic Signature"}],
  "graphql-introspection":[{framework:"OWASP-Top10",control:"A05:2021",desc:"Security Misconfiguration"},{framework:"CWE",control:"CWE-200",desc:"Exposure of Sensitive Information"}],
  "exposed-debug":[{framework:"PCI-DSS-v4",control:"2.2.4",desc:"Only necessary services enabled"},{framework:"NESA-UAE-IAS",control:"T7.4.3",desc:"Web application security"},{framework:"SAMA-CSF",control:"3.3.5",desc:"Application security"},{framework:"ISO-27001",control:"A.12.1.4",desc:"Separation of dev/test/prod"},{framework:"OWASP-Top10",control:"A05:2021",desc:"Security Misconfiguration"},{framework:"CWE",control:"CWE-489",desc:"Active Debug Code"}],
  "directory-listing":[{framework:"OWASP-Top10",control:"A05:2021",desc:"Security Misconfiguration"},{framework:"CWE",control:"CWE-548",desc:"Exposure Through Directory Listing"}],
  "weak-tls":[{framework:"PCI-DSS-v4",control:"4.2.1",desc:"Strong cryptography in transit"},{framework:"NESA-UAE-IAS",control:"T2.3.5",desc:"Secure communications"},{framework:"SAMA-CSF",control:"3.3.14",desc:"Cryptography"},{framework:"DESC-ISR",control:"CO.2.1",desc:"Cryptographic protection"},{framework:"ISO-27001",control:"A.10.1.1",desc:"Cryptographic controls policy"},{framework:"OWASP-Top10",control:"A02:2021",desc:"Cryptographic Failures"},{framework:"CWE",control:"CWE-326",desc:"Inadequate Encryption Strength"}],
  "mixed-content":[{framework:"OWASP-Top10",control:"A02:2021",desc:"Cryptographic Failures"},{framework:"CWE",control:"CWE-311",desc:"Missing Encryption of Sensitive Data"}],
  "missing-sri":[{framework:"OWASP-Top10",control:"A06:2021",desc:"Vulnerable Components"},{framework:"CWE",control:"CWE-353",desc:"Missing Support for Integrity Check"}],
  "verbose-error":[{framework:"OWASP-Top10",control:"A04:2021",desc:"Insecure Design"},{framework:"PCI-DSS-v4",control:"6.2.4",desc:"Software protected against attacks"},{framework:"CWE",control:"CWE-209",desc:"Generation of Error Message Containing Sensitive Information"}],
  "exposed-env":[{framework:"PCI-DSS-v4",control:"3.3.1",desc:"Sensitive auth data not retained"},{framework:"NESA-UAE-IAS",control:"T6.1.7",desc:"Information leakage prevention"},{framework:"SAMA-CSF",control:"3.3.10",desc:"Identity and access management"},{framework:"ISO-27001",control:"A.9.4.3",desc:"Password management"},{framework:"OWASP-Top10",control:"A05:2021",desc:"Security Misconfiguration"},{framework:"CWE",control:"CWE-200",desc:"Exposure of Sensitive Information"}],
  "broken-access-control":[{framework:"PCI-DSS-v4",control:"7.2.1",desc:"Access controls / least privilege"},{framework:"NESA-UAE-IAS",control:"T1.3.5",desc:"Authorization controls"},{framework:"SAMA-CSF",control:"3.3.10",desc:"Identity and access management"},{framework:"DESC-ISR",control:"AM.2.1",desc:"Access management"},{framework:"ISO-27001",control:"A.9.4.1",desc:"Information access restriction"},{framework:"OWASP-Top10",control:"A01:2021",desc:"Broken Access Control"},{framework:"CWE",control:"CWE-285",desc:"Improper Authorization"}],
  "idor-confirmed":[{framework:"OWASP-Top10",control:"A01:2021",desc:"Broken Access Control"},{framework:"PCI-DSS-v4",control:"7.2.1",desc:"Least privilege"},{framework:"CWE",control:"CWE-639",desc:"Authorization Bypass via User-Controlled Key"}],
  "rate-limit":[{framework:"OWASP-Top10",control:"A04:2021",desc:"Insecure Design"},{framework:"PCI-DSS-v4",control:"8.3.6",desc:"Account lockout / rate limit"},{framework:"CWE",control:"CWE-307",desc:"Improper Restriction of Excessive Auth Attempts"}],
};
function getComplianceForFinding(f){if(!f)return [];const key=f._fixKey||f.type;return COMPLIANCE_MAP[key]||[];}
function showDiffPanel(data){
  const wrap=document.getElementById("bluPanels");
  if(!wrap)return;
  chrome.runtime.sendMessage({action:"diffSnapshots",tabId,host:(()=>{try{return new URL(data.url||"").hostname;}catch(e){return "";}})()},r=>{
    if(!r||!r.ok){wrap.innerHTML=`<div style="padding:14px 16px;border-top:1px solid var(--glassbrd);background:var(--bgh);font-size:11px;color:var(--t3)">No snapshots yet — click <strong>Snapshot</strong> first to capture a baseline.</div>`;return;}
    const d=r.diff||{new:[],resolved:[],unchanged:[]};
    if(r.last)_bluLastSnapMeta={host:r.host,score:r.last.score||100,ts:r.last.ts};
    let html=`<div style="padding:14px 16px;border-top:1px solid var(--glassbrd);background:var(--bgh)">
      <div style="font-size:11px;color:var(--purple);font-weight:700;text-transform:uppercase;letter-spacing:.5px;margin-bottom:8px">Comparison: now vs ${r.last?new Date(r.last.ts).toLocaleString():"baseline"}</div>`;
    html+=`<div style="font-size:10px;color:var(--red);margin-top:6px;font-weight:700">Introduced this release (${d.new.length})</div>`;
    if(d.new.length){d.new.forEach(f=>{html+=`<div style="font-family:var(--mono);font-size:10px;color:var(--t2);padding:2px 0">◆ ${esc(f.severity||"")} ${esc(f.type||"")} — ${esc(f.where||"")}</div>`;});}
    else{html+=`<div style="font-size:10px;color:var(--t3)">— none —</div>`;}
    html+=`<div style="font-size:10px;color:var(--green);margin-top:8px;font-weight:700">Fixed since last scan (${d.resolved.length})</div>`;
    if(d.resolved.length){d.resolved.forEach(f=>{html+=`<div style="font-family:var(--mono);font-size:10px;color:var(--t3);padding:2px 0">✓ ${esc(f.type||"")} — ${esc(f.where||"")}</div>`;});}
    else{html+=`<div style="font-size:10px;color:var(--t3)">— none —</div>`;}
    html+=`<div style="font-size:10px;color:var(--t3);margin-top:8px;font-weight:700">Unchanged (${d.unchanged.length})</div>`;
    html+=`<div style="display:flex;gap:6px;margin-top:10px">
      <button class="btn v6-focusable" id="bluDiffMd">Export Diff Markdown</button>
      <button class="btn v6-focusable" id="bluDiffClose">Close</button>
    </div></div>`;
    wrap.innerHTML=html;
    document.getElementById("bluDiffMd")?.addEventListener("click",()=>{
      const md=`# PenScope Diff\n\n## Introduced (${d.new.length})\n${d.new.map(f=>`- ${f.severity} ${f.type} — ${f.where}`).join("\n")}\n\n## Resolved (${d.resolved.length})\n${d.resolved.map(f=>`- ${f.type} — ${f.where}`).join("\n")}\n\n## Unchanged (${d.unchanged.length})\n${d.unchanged.map(f=>`- ${f.type}`).join("\n")}\n`;
      navigator.clipboard.writeText(md).then(()=>toast("Diff Markdown copied"),()=>toast("Clipboard blocked"));
    });
    document.getElementById("bluDiffClose")?.addEventListener("click",()=>{wrap.innerHTML="";});
  });
}

// v6.0 — Mode router internals. setMode() is the public toggle entry point: validates,
// updates local state, swaps the body class so the CSS variable theme switches, persists
// to background, then re-renders. The persistence write is fire-and-forget — UI updates
// immediately on click without waiting for the round trip.
function setMode(mode){
  if(!MODES.includes(mode))return;
  if(mode===currentMode){
    // Already in this mode — re-render anyway in case data changed
    rerender();
    return;
  }
  currentMode=mode;
  applyModeBodyClass(mode);
  syncModeToggleUI(mode);
  try{chrome.runtime.sendMessage({action:"setMode",tabId,mode},()=>{/* fire-and-forget */});}catch(e){/* SW may be cold-starting; the next load() will pick up state from getData */}
  rerender();
}

// Body class swap is what activates the CSS variable override. Removing all mode-* and
// setting just one keeps the body class clean even if old classes leaked in somehow.
function applyModeBodyClass(mode){
  const b=document.body;
  b.classList.remove("mode-classic","mode-red","mode-blue");
  b.classList.add("mode-"+mode);
}

// Keep the 3-segment pill in sync with currentMode. aria-selected controls both the
// visual state (filled gradient) and screen-reader announcements.
...
function parseCustomHeaders(text){
  ...
  text.split(/\r?\n/).forEach(line=>{
    ...
    const name=line.substring(0,colonIdx).trim().replace(/[\r\n\0]/g,"");
...
function startProbeWithLevel(level){
  ...
  const levelLabels={careful:"🟢 Careful — GET only, sequential",medium:"🟡 Medium — POST for introspection/auth tests",full:"🔴 Full Send — all methods, race bursts"};
  const hdrEl=document.getElementById("probeHeaders");
  const customHeaders=parseCustomHeaders(hdrEl?.value||"");
...
let _epRouteView=false;
function _normalizePathPattern(path){
  if(!path)return path;
  return path.split("?")[0].split("/").map(seg=>{
    ...
    if(/^[A-Za-z0-9]{20,}$/.test(seg)&&!/[._-]/.test(seg))return "{hash}";
    return seg;
  }).join("/");
}
function _buildCurl(e,reqHeaders){
  const method=e.method||"GET";
  const url=e.url||"";
  let cmd=`curl -i -X ${method} '${url.replace(/'/g,"'\\''")}'`;
  const hdrs=reqHeaders[url]||[];
  hdrs.forEach(h=>{const safe=String(h.value||"").replace(/'/g,"'\\''");cmd+=` \\\n  -H '${h.name}: ${safe}'`;});
  return cmd;
}
function _autoPivotSuggestions(){
  const subs=Array.isArray(D.subdomains)?D.subdomains:[];
  if(!subs.length)return[];
  let curHost="";try{curHost=new URL(D.url||"").hostname;}catch(e){}
  const SENS=/^(?:admin|admin-?api|internal|api-?internal|api|api-?dev|api-?staging|api-?stg|backend|management|portal|console|dashboard|jenkins|grafana|kibana|gitlab|jira|airflow|argocd)\b/i;
  return subs.map(s=>typeof s==="string"?s:(s&&s.host)||"").filter(host=>{
    if(!host||host===curHost)return false;
    const first=String(host).split(".")[0]||"";
    return SENS.test(first);
  }).slice(0,8);
}
function renderEndpoints(eps,filter=""){
  const c=document.getElementById("rE");
  let list=eps;
  if(filter){
    const f=filter.toLowerCase();
    list=eps.filter(e=>e.path.toLowerCase().includes(f)||e.method.toLowerCase().includes(f)||e.type.toLowerCase().includes(f)||(e.status&&String(e.status).includes(f))||(e.tags&&e.tags.some(t=>t.tag.includes(f))));
  }
  if(!list.length){c.innerHTML=empty("📡","No endpoints.");return;}
  const reqHeaders={};
  (D.requestHeaders||[]).forEach(h=>{if(h&&h.url&&!reqHeaders[h.url])reqHeaders[h.url]=h.headers||[];});
  let h="";
  const pivots=_autoPivotSuggestions();
  if(pivots.length){
    h+=`<div class="ep-grp" style="border-left:2px solid var(--coral)"><div class="ep-grp-t" style="color:var(--coral)">→ Pivot candidates (${pivots.length})</div>`;
    pivots.forEach(host=>{
      const url=`https://${host}/`;
      h+=`<div class="ep" data-pivot="${escA(url)}" style="cursor:pointer"><span class="m m-GET" style="background:var(--coral);color:#fff">PIVOT</span><span class="ep-p" style="color:var(--coral)">${esc(host)}</span><span style="color:var(--t3);font-size:9px">→ click to open in new tab</span></div>`;
    });
    h+=`</div>`;
  }
  h+=`<div style="display:flex;gap:6px;padding:6px 14px;font-size:10px;align-items:center;background:var(--bgh);border-bottom:1px solid var(--glassbrd)">
    <span style="color:var(--t3)">View:</span>
    <button class="btn ${_epRouteView?'':'active'}" id="epViewLiteral" style="padding:3px 8px;font-size:10px">Literal (${list.length})</button>
    <button class="btn ${_epRouteView?'active':''}" id="epViewRoute" style="padding:3px 8px;font-size:10px">Route table</button>
  </div>`;
  if(_epRouteView){
    const routes={};
    list.forEach(e=>{
      const pat=_normalizePathPattern(e.path);
      const key=`${e.host}|${e.method}|${pat}`;
      if(!routes[key])routes[key]={host:e.host,method:e.method,pattern:pat,count:0,statuses:new Set(),samples:[],tags:e.tags||[]};
      routes[key].count++;
      if(e.status)routes[key].statuses.add(e.status);
      if(routes[key].samples.length<5)routes[key].samples.push(e.url);
    });
    const grouped={};
    Object.values(routes).forEach(r=>{(grouped[r.host]=grouped[r.host]||[]).push(r);});
    for(const[host,rs] of Object.entries(grouped)){
      h+=`<div class="ep-grp"><div class="ep-grp-t">${esc(host)} (${rs.length} routes)</div>`;
      rs.sort((a,b)=>b.count-a.count).forEach(r=>{
        const tags=(r.tags||[]).map(t=>`<span class="ep-tag" style="background:${escA(t.color)}22;color:${escA(t.color)}">${esc(t.tag)}</span>`).join("");
        const stat=[...r.statuses].slice(0,4).map(s=>`<span class="ep-status ${statusClass(s)}">${s}</span>`).join("");
        h+=`<div class="ep" data-route="${escA(r.samples[0]||"")}"><span class="m m-${escA(r.method)}">${r.method}</span>${stat}<span class="ep-p">${esc(r.pattern)}</span>${tags}<span class="ep-size">${r.count}×</span></div>`;
      });
      h+=`</div>`;
    }
  }else{
    const groups={};
    list.forEach(e=>{(groups[e.host]=groups[e.host]||[]).push(e);});
    for(const[host,es]of Object.entries(groups)){
      h+=`<div class="ep-grp"><div class="ep-grp-t">${esc(host)} (${es.length})</div>`;
      es.forEach((e,idx)=>{
        const tags=(e.tags||[]).map(t=>`<span class="ep-tag" style="background:${escA(t.color)}22;color:${escA(t.color)}">${esc(t.tag)}</span>`).join("");
        h+=`<div class="ep" data-u="${escA(e.url)}" data-curl="${escA(_buildCurl(e,reqHeaders))}"><span class="m m-${escA(e.method)}">${e.method}</span>`;
        if(e.status)h+=`<span class="ep-status ${statusClass(e.status)}">${e.status}</span>`;
        h+=`<span class="ep-p">${esc(e.path)}${e.query?`<span style="color:var(--t3)">${esc(e.query.substring(0,40))}</span>`:""}</span>${tags}`;
        if(e.responseSize)h+=`<span class="ep-size">${fmtSize(e.responseSize)}</span>`;
        h+=`<span class="ep-t">${e.type}</span>`;
        h+=`<button class="btn ep-curl-btn" style="padding:1px 6px;font-size:9px;margin-left:4px" title="Copy as curl">curl</button>`;
        if(D.deepEnabled){h+=`<button class="btn ep-replay-btn" data-replay-method="${escA(e.method||"GET")}" data-replay-url="${escA(e.url)}" style="padding:1px 6px;font-size:9px;margin-left:2px;color:var(--orange)" title="Replay without cookies">unauth</button>`;}
        h+=`</div>`;
      });
      h+=`</div>`;
    }
  ...
  c.innerHTML=h;
  document.getElementById("epViewLiteral")?.addEventListener("click",()=>{_epRouteView=false;renderEndpoints(eps,filter);});
  document.getElementById("epViewRoute")?.addEventListener("click",()=>{_epRouteView=true;renderEndpoints(eps,filter);});
  c.querySelectorAll(".ep[data-u]").forEach(el=>el.addEventListener("click",ev=>{
    if(ev.target.classList.contains("ep-curl-btn")){ev.stopPropagation();copy(el.dataset.curl||"");return;}
    if(ev.target.classList.contains("ep-replay-btn")){ev.stopPropagation();_runUnauthReplay(el.dataset.replayMethod||"GET",el.dataset.replayUrl||el.dataset.u);return;}
    copy(el.dataset.u);
  }));
  c.querySelectorAll(".ep[data-pivot]").forEach(el=>el.addEventListener("click",()=>{
    chrome.tabs.create({url:el.getAttribute("data-pivot")||""},()=>{void chrome.runtime.lastError;});
  }));
  c.querySelectorAll(".ep[data-route]").forEach(el=>el.addEventListener("click",()=>copy(el.getAttribute("data-route")||"")));
}
function _runUnauthReplay(method,url){
  if(!url){toast("No URL");return;}
  toast("Replaying without cookies...");
  chrome.runtime.sendMessage({action:"replayUnauth",tabId,method,url},r=>{
    if(chrome.runtime.lastError){toast("Replay failed: "+chrome.runtime.lastError.message);return;}
    if(!r||!r.ok){toast("Replay error: "+(r?.error||"unknown"));return;}
    const cred=r.withCookies||{};const omit=r.withoutCookies||{};
    const sameStatus=cred.status===omit.status;
    const sameSize=Math.abs((cred.size||0)-(omit.size||0))<50;
    let verdict="";
    if(cred.status===0||omit.status===0){
      const errLines=[cred.error?"with-cookies error: "+cred.error:null,omit.error?"without-cookies error: "+omit.error:null].filter(Boolean).join("\n");
      verdict="⚠ Network error — verify manually"+(errLines?"\n"+errLines:"");
    }else if(sameStatus&&sameSize){
      verdict="🚨 IDENTICAL — auth not enforced";
    }else if(omit.status>=200&&omit.status<300){
      verdict="⚠️ unauth returns 2xx — partial bypass";
    }else{
      verdict="✓ auth required";
    }
    const worldNote=r._world?`  [via ${r._world} world]`:"";
    const msg=`Replay ${method} ${url}${worldNote}\nWith cookies: ${cred.status||"err"} (${cred.size||0}B)\nWithout cookies: ${omit.status||"err"} (${omit.size||0}B)\n${verdict}`;
    window.alert(msg);
  });
}
...
const stRes=D.probeData?.subdomainTakeoverResults||[];
if(stRes.length){
  h+=`<div class="hs" style="border-left:2px solid var(--red)"><div class="hs-t" style="color:var(--red)">🪤 Subdomain Takeover Heuristics (${stRes.length})</div>`;
  stRes.forEach(r=>{
    h+=`<div class="fi"><div class="fi-h"><span class="sev sev-${escA(r.severity||"high")}">${esc(r.severity||"high")}</span><span class="fi-t">${esc(r.host)}</span><span style="margin-left:auto;color:var(--coral);font-size:10px">${esc(r.service||"")}</span></div><div class="fi-v" style="cursor:pointer">${esc(r.note||"")}</div><div class="fi-m" style="font-family:var(--mono);font-size:9px;max-height:60px;overflow:auto">${esc((r.bodyPreview||"").substring(0,200))}</div></div>`;
  });
  h+=`</div>`;
}
const tabNab=D.probeData?.tabNabbingResults||[];
if(tabNab.length){
  h+=`<div class="hs"><div class="hs-t" style="color:var(--orange)">🔓 TabNabbing Risk</div>`;
  tabNab.forEach(r=>{
    h+=`<div class="fi"><div class="fi-h"><span class="sev sev-${escA(r.severity||"medium")}">${esc(r.severity||"medium")}</span><span class="fi-t">${r.count} cross-origin _blank links missing rel=noopener</span></div><div class="fi-m">${esc(r.note||"")}</div>`;
    (r.examples||[]).slice(0,5).forEach(e=>{h+=`<div style="padding:2px 14px;font-family:var(--mono);font-size:9px;color:var(--t3)">→ ${esc(e.host)}: ${esc(e.text||"(no text)")}</div>`;});
    h+=`</div>`;
  });
  h+=`</div>`;
}
const pmRisk=D.probeData?.postMessageRiskResults||[];
if(pmRisk.length){
  h+=`<div class="hs"><div class="hs-t" style="color:var(--red)">postMessage Origin-Check Correlation</div>`;
  pmRisk.forEach(r=>{h+=`<div class="fi"><div class="fi-h"><span class="sev sev-${escA(r.severity||"info")}">${esc(r.severity||"info")}</span><span class="fi-t">${r.totalListeners} listener(s), ${r.withoutOriginCheck} unsafe</span></div><div class="fi-m">${esc(r.note||"")}</div></div>`;});
  h+=`</div>`;
}
const swScope=D.probeData?.swScopeResults||[];
if(swScope.length){
  h+=`<div class="hs"><div class="hs-t" style="color:var(--orange)">Service Worker Scope Audit</div>`;
  swScope.forEach(r=>{h+=`<div class="fi"><div class="fi-h"><span class="sev sev-${escA(r.severity||"low")}">${esc(r.severity||"low")}</span><span class="fi-t">${esc(r.scope)}</span></div><div class="fi-m" style="font-family:var(--mono);font-size:9px">${esc(r.scriptUrl||"")} — ${esc(r.note||"")}</div></div>`;});
  h+=`</div>`;
}
const ssrf=D.probeData?.ssrfResults||[];
if(ssrf.length){
  h+=`<div class="hs"><div class="hs-t" style="color:var(--red)">SSRF Param Probing</div>`;
  ssrf.forEach(r=>{h+=`<div class="fi"><div class="fi-h"><span class="sev sev-${escA(r.severity||"medium")}">${esc(r.severity||"medium")}</span><span class="fi-t">${esc(r.path)}?${esc(r.param)}=</span></div><div class="fi-v" style="font-family:var(--mono);font-size:9px">${esc(r.payload||"")}</div><div class="fi-m">${esc(r.note||"")}</div></div>`;});
  h+=`</div>`;
}
const nosql=D.probeData?.nosqlResults||[];
if(nosql.length){
  h+=`<div class="hs"><div class="hs-t" style="color:var(--coral)">NoSQL Operator Injection</div>`;
  nosql.forEach(r=>{h+=`<div class="fi"><div class="fi-h"><span class="sev sev-${escA(r.severity||"medium")}">${esc(r.severity||"medium")}</span><span class="fi-t">${esc(r.path)} (${esc(r.field)} → ${esc(r.operator)})</span></div><div class="fi-m">size delta ${r.sizeDelta} between baseline and injected</div></div>`;});
  h+=`</div>`;
}
const cdR=D.probeData?.cacheDeceptionResults||[];
if(cdR.length){
  h+=`<div class="hs"><div class="hs-t" style="color:var(--coral)">Cache Deception</div>`;
  cdR.forEach(r=>{h+=`<div class="fi"><div class="fi-h"><span class="sev sev-${escA(r.severity||"medium")}">${esc(r.severity||"medium")}</span><span class="fi-t">${esc(r.deceitPath)}</span></div><div class="fi-m" style="font-family:var(--mono);font-size:9px">CT: ${esc(r.contentType||"")} — ${esc(r.note||"")}</div></div>`;});
  h+=`</div>`;
}
const gqlA=D.probeData?.gqlAliasResults||[];
const gqlB=D.probeData?.gqlBatchResults||[];
if(gqlA.length||gqlB.length){
  h+=`<div class="hs"><div class="hs-t">GraphQL DoS Surface</div>`;
  gqlA.forEach(r=>{h+=`<div class="fi"><div class="fi-h"><span class="sev sev-${escA(r.severity||"info")}">alias</span><span class="fi-t">${r.aliasCount} aliases — ${r.timeMs}ms — ${r.typenames} fields</span></div><div class="fi-m">${esc(r.note||"")}</div></div>`;});
  gqlB.forEach(r=>{h+=`<div class="fi"><div class="fi-h"><span class="sev sev-${escA(r.severity||"info")}">batch</span><span class="fi-t">${r.batchSize} batched — ${r.timeMs}ms</span></div><div class="fi-m">${esc(r.note||"")}</div></div>`;});
  h+=`</div>`;
}

if(D.symbolTable?.length){
  const byFile={};
  D.symbolTable.forEach(s=>{
    const file=(s.source||s.file||"unknown").split("/").pop();
    if(!byFile[file])byFile[file]={count:0,sensitive:[],all:new Set()};
    byFile[file].count++;
    byFile[file].all.add(s.name||s);
    const nm=String(s.name||s);
    if(/admin|role|permission|privilege|impersonate|sudo|owner|debug|internal|secret|root/i.test(nm))byFile[file].sensitive.push(nm);
  });
  const fileEntries=Object.entries(byFile).filter(([,v])=>v.sensitive.length>0).sort((a,b)=>b[1].sensitive.length-a[1].sensitive.length).slice(0,20);
  if(fileEntries.length){
    h+=`<div class="hs"><div class="hs-t" style="color:var(--purple)">📚 Source-map Symbol Clusters (auth-shaped)</div>`;
    fileEntries.forEach(([file,info])=>{
      h+=`<div class="fi"><div class="fi-h"><span class="sev sev-medium">${info.sensitive.length}</span><span class="fi-t">${esc(file)}</span><span style="margin-left:auto;color:var(--t3);font-size:9px">${info.count} symbols</span></div><div class="fi-v" style="font-family:var(--mono);font-size:9px;color:var(--coral)">${info.sensitive.slice(0,15).map(esc).join(", ")}${info.sensitive.length>15?` …+${info.sensitive.length-15}`:""}</div></div>`;
    });
    h+=`</div>`;
  }
}

h+=`<div class="hs" id="hsIdb" style="border-left:2px solid var(--teal)"><div class="hs-t" style="color:var(--teal)">💾 IndexedDB Snapshots / Diff</div>
  <div style="padding:6px 14px;display:flex;gap:6px;flex-wrap:wrap">
    <button class="btn" id="btnIdbCapture" style="padding:3px 8px;font-size:10px">📸 Capture now</button>
    <button class="btn" id="btnIdbDiff" style="padding:3px 8px;font-size:10px">↔ Diff vs prior</button>
    <span id="idbStatus" style="font-size:10px;color:var(--t3);align-self:center"></span>
  </div>
  <div id="idbResult" style="font-family:var(--mono);font-size:10px;padding:0 14px;max-height:200px;overflow:auto"></div>
</div>`;

if(D.spaNavigations?.length){
  h+=`<div class="hs"><div class="hs-t">SPA Navigations (${D.spaNavigations.length})</div>`;
  D.spaNavigations.slice(-15).reverse().forEach(n=>{
    const dt=new Date(n.ts).toLocaleTimeString();
    h+=`<div style="padding:2px 14px;font-family:var(--mono);font-size:9px;color:${n.originChanged?"var(--orange)":"var(--t2)"}"><span style="color:var(--t3)">${dt}</span> ${esc((n.from||"").substring(0,50))} → ${esc((n.to||"").substring(0,50))}${n.originChanged?" ⚠ origin changed":""}</div>`;
  });
  h+=`</div>`;
}

if(D.ariaLabels?.length){
  const sens=D.ariaLabels.filter(a=>a.sensitive);
  if(sens.length){
    h+=`<div class="hs"><div class="hs-t" style="color:var(--coral)">🔍 Sensitive Aria Labels (${sens.length})</div>`;
    sens.slice(0,40).forEach(a=>{h+=`<div class="fi"><div class="fi-h"><span class="sev sev-medium">${esc(a.role||"-")}</span><span class="fi-t">${esc(a.label)}</span></div></div>`;});
    h+=`</div>`;
  }
}

if(D.partitionedCookies?.length){
  h+=`<div class="hs"><div class="hs-t">CHIPS Partitioned Cookies (${D.partitionedCookies.length})</div>`;
  D.partitionedCookies.slice(0,30).forEach(c=>{
    h+=`<div style="padding:3px 14px;font-family:var(--mono);font-size:9px;color:var(--t2)">${esc(c.name)}@${esc(c.domain)} <span style="color:var(--t3)">partition=${esc(JSON.stringify(c.partitionKey).substring(0,80))}</span></div>`;
  });
  h+=`</div>`;
}

if(D.jsonIslands?.length){
  h+=`<div class="hs"><div class="hs-t">JSON Islands (${D.jsonIslands.length})</div>`;
  D.jsonIslands.forEach(isl=>{
    h+=`<div class="fi"><div class="fi-h"><span class="sev sev-info">${esc(isl.tag||"json")}</span><span class="fi-t">${esc(isl.id||"")}</span><span style="margin-left:auto;color:var(--t3);font-size:9px">${fmtSize(isl.size||0)} · ${(isl.endpoints||[]).length} eps · ${(isl.ids||[]).length} ids</span></div>`;
    (isl.endpoints||[]).slice(0,8).forEach(ep=>{h+=`<div style="padding:1px 14px;font-family:var(--mono);font-size:9px;color:var(--blue)">${esc(ep)}</div>`;});
    h+=`</div>`;
  });
  h+=`</div>`;
}
...
document.getElementById("btnIdbCapture")?.addEventListener("click",()=>{
  document.getElementById("idbStatus").textContent="Capturing...";
  chrome.runtime.sendMessage({action:"captureIdb",tabId},r=>{
    if(chrome.runtime.lastError){document.getElementById("idbStatus").textContent="Error: "+chrome.runtime.lastError.message;return;}
    if(!r||!r.ok){document.getElementById("idbStatus").textContent="Failed: "+(r?.error||"unknown");return;}
    document.getElementById("idbStatus").textContent=`${r.count} snapshot(s); newest: ${(r.snapshot.dbs||[]).length} dbs`;
    const out=document.getElementById("idbResult");
    out.textContent=JSON.stringify(r.snapshot,null,2).substring(0,4000);
  });
});
document.getElementById("btnIdbDiff")?.addEventListener("click",()=>{
  document.getElementById("idbStatus").textContent="Diffing...";
  chrome.runtime.sendMessage({action:"diffIdb",tabId},r=>{
    if(chrome.runtime.lastError){document.getElementById("idbStatus").textContent="Error: "+chrome.runtime.lastError.message;return;}
    if(!r||!r.ok){document.getElementById("idbStatus").textContent=r?.error||"failed";return;}
    const d=r.diff||{};
    const summary=`+${d.added.length} added · -${d.removed.length} removed · ~${d.changed.length} changed`;
    document.getElementById("idbStatus").textContent=summary;
    document.getElementById("idbResult").textContent=JSON.stringify(d,null,2).substring(0,4000);
  });
});
...
function updateFooter(){
  if(D?.startTime){const s=Math.floor((Date.now()-D.startTime)/1000);document.getElementById("fTime").textContent=`${Math.floor(s/60)}m ${s%60}s`;}
  // v6.0 — Footer label reflects the active mode so users have ambient feedback about
  // which renderer is running. Classic gets the v5.9-equivalent line; Red/Blue tease
  // the mode-specific functionality.
  const fl=document.getElementById("footerLabel");
  if(fl){
    if(currentMode==="red")fl.textContent="PenScope v6.0 — Red mode · chain weaponize · stack packs · Claude bidirectional sync";
    else if(currentMode==="blue")fl.textContent="PenScope v6.0 — Blue mode · health score · top-5 fixes · CSP gen · 7-framework compliance";
    else fl.textContent="PenScope v6.0 — Classic mode (v5.9 verbatim) · Red/Blue modes available";
  }
}
...
navigator.clipboard.writeText(r).then(()=>{
  ...
  const ok=window.confirm("Clipboard unavailable. Save report to disk?\n\nThe file will contain captured response bodies, header dumps, and may include sensitive data from the target. Save only if you trust the local Downloads folder.\n\nProceed with download?");
  if(!ok){toast("Download cancelled");return;}
  download("penscope-report.md",r,"text/markdown");
  toast("Downloaded — too large for clipboard");
});}
...
case"burp":{let t="";D.endpoints?.forEach(e=>t+=e.url+"\n");
  ...
  case"har":{downloadHarExport(host);break;}
  case"ffuf":{downloadFfufWordlist(host);break;}
}}

function downloadFfufWordlist(host){
  const params=new Set();
  (D.params||[]).forEach(p=>{if(p.param)params.add(p.param);});
  D.forms?.forEach(f=>f.inputs?.forEach(i=>{if(i.name)params.add(i.name);}));
  D.hiddenFields?.forEach(f=>{if(f.name&&f.name!=="<!-- -->")params.add(f.name);});
  if(!params.size){toast("No parameters to export");return;}
  const lines=[...params].sort().map(p=>`${p}=FUZZ`);
  download(`penscope_${host}_params_ffuf.txt`,lines.join("\n"),"text/plain");
  toast(`Exported ${params.size} parameters as ffuf wordlist`);
}

function downloadHarExport(host){
  const eps=D.endpoints||[];
  if(!eps.length){toast("No endpoints to export");return;}
  const postBodies={};
  (D.postBodies||[]).forEach(p=>{const k=`${p.method}:${p.path}`;if(!postBodies[k])postBodies[k]=p;});
  const reqHeaders={};
  (D.requestHeaders||[]).forEach(h=>{
    if(!h||!h.url)return;
    if(!reqHeaders[h.url])reqHeaders[h.url]=h.headers||[];
  });
  const startTime=D.startTime||Date.now();
  const entries=eps.map(e=>{
    const ts=e.timestamp||startTime;
    const queryParts=[];
    if(e.query){
      try{new URLSearchParams(e.query.startsWith("?")?e.query.substring(1):e.query).forEach((v,k)=>{queryParts.push({name:k,value:v});});}catch{}
    }
    const headers=(reqHeaders[e.url]||[]).map(h=>({name:h.name,value:h.value}));
    const bodyEntry=postBodies[`${e.method}:${e.path}`];
    const req={method:e.method||"GET",url:e.url,httpVersion:"HTTP/1.1",headers,queryString:queryParts,cookies:[],headersSize:-1,bodySize:bodyEntry?(bodyEntry.body||"").length:0};
    if(bodyEntry&&bodyEntry.body)req.postData={mimeType:bodyEntry.contentType||"application/json",text:bodyEntry.body};
    return{
      ...
      response:{
        status:e.status||0,
        statusText:e.status?String(e.status):"",
        httpVersion:"HTTP/1.1",
        cookies:[],headers:[],
        content:{size:e.responseSize||0,mimeType:""},
        redirectURL:"",
        headersSize:-1,bodySize:e.responseSize||0
      },
      cache:{},
      timings:{send:0,wait:0,receive:0},
      _penscope:{tags:(e.tags||[]).map(t=>t.tag),initiator:e.initiator||"",type:e.type||""}
    };
  });
  const har={
    log:{
      version:"1.2",
      creator:{name:"PenScope",version:"6.2.5",comment:"Exported from PenScope passive capture"},
      browser:{name:"Chrome",version:"unknown"},
      pages:[{startedDateTime:new Date(startTime).toISOString(),id:"page_1",title:host,pageTimings:{onContentLoad:-1,onLoad:-1}}],
      entries
    }
  };
  download(`penscope_${host}.har`,JSON.stringify(har,null,2),"application/json");
  toast(`Exported ${entries.length} entries to HAR`);
}
